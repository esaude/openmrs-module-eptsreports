...
public class InitialArtStartDateCalculationTest extends BasePatientCalculationTest {
	...
	public CalculationResultMap getResult() {
		...
		map.put(2, new SimpleResult(new Timestamp(calculationsTestsCache.getDate("2008-08-01 00:00:00.0").getTime()),
		        calculation, evaluationContext));
		// initiated ART by starting ARV plan observation
		map.put(6, new SimpleResult(new Timestamp(calculationsTestsCache.getDate("2018-10-21 00:00:00.0").getTime()),
		        calculation, evaluationContext));
		// initiated ART by historical start date observation
		map.put(7, new SimpleResult(new Timestamp(calculationsTestsCache.getDate("2019-01-18 00:00:00.0").getTime()),
		        calculation, evaluationContext));
		// initiated ART by first phamarcy encounter observation
		map.put(8, new SimpleResult(new Timestamp(calculationsTestsCache.getDate("2019-01-21 00:00:00.0").getTime()),
		        calculation, evaluationContext));
		// initiated ART by ARV transfer in observation
		map.put(999, new SimpleResult(new Timestamp(calculationsTestsCache.getDate("2019-01-20 00:00:00.0").getTime()),
		        calculation, evaluationContext));
	...
	public void evaluate_shouldConsiderLeastInitiationDateForMultipleEntryPoints() {
		...
		// enrollment
		calculationsTestsCache.createBasicObs(Context.getPatientService().getPatient(2), Context.getConceptService()
		        .getConcept(7777002), Context.getEncounterService().getEncounter(3), calculationsTestsCache
		        .getDate("2008-07-01 00:00:00.0"), (Location) getEvaluationContext().getFromCache("location"), Context
		        .getConceptService().getConcept(7777003));
		CalculationResultMap evaluatedResult = service.evaluate(getCohort(), getCalculation(), getEvaluationContext());
		Assert.assertEquals(calculationsTestsCache.getDate("2008-07-01 00:00:00.0"), evaluatedResult.get(2).getValue());
		...
		calculationsTestsCache.createBasicObs(Context.getPatientService().getPatient(2), Context.getConceptService()
		        .getConcept(7777005), Context.getEncounterService().getEncounter(3), calculationsTestsCache
		        .getDate("2019-01-01 00:00:00.0"), (Location) getEvaluationContext().getFromCache("location"),
		...
		Assert.assertEquals(calculationsTestsCache.getDate("2008-06-01 00:00:00.0"), evaluatedResult.get(2).getValue());
		// the rest should not be touched
		matchOtherResultsExcept(evaluatedResult, 2);
		...
		calculationsTestsCache.createBasicObs(Context.getPatientService().getPatient(2), Context.getConceptService()
		        .getConcept(7777002), Context.getEncounterService().getEncounter(3), calculationsTestsCache
		        .getDate("2019-01-02 00:00:00.0"), (Location) getEvaluationContext().getFromCache("location"), Context
		        .getConceptService().getConcept(7777004));
		evaluatedResult = service.evaluate(getCohort(), getCalculation(), getEvaluationContext());
		Assert.assertEquals(calculationsTestsCache.getDate("2008-06-01 00:00:00.0"), evaluatedResult.get(2).getValue());
		// the rest should not be touched
		matchOtherResultsExcept(evaluatedResult, 2);
