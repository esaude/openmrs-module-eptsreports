...
public class CalculationCohortDefinitionEvaluatorTest extends BaseContextMockTest {
    ...
    @Spy
    private PatientCalculationService patientCalculationService;
    ...
    public void setUp() {
        ...
        when(Context.getService(PatientSetService.class)).thenReturn(patientSetService);
    ...
    public void evaluateShouldReturnAnEvaluatedCohort() throws EvaluationException {
        PatientCalculationContext calculationContext = patientCalculationService.createCalculationContext();
        ...
        when(EptsCalculationUtils.patientsThatPass(resultMap, definition.getWithResult(), definition.getWithResultFinder(), context)).thenReturn(patients.getMemberIds());
        when(patientCalculationService.evaluate(patients.getMemberIds(), definition.getCalculation(), definition.getCalculationParameters(), calculationContext)).thenReturn(resultMap);
        EvaluatedCohort evaluatedCohort = evaluator.evaluate(definition, context);
        Assert.assertEquals(evaluatedCohort.getMemberIds(), patients);
    ...
    @Test
    public void evaluateShouldEvaluateCalculationWithAllPatientsIfNoBaseCohortIsSet() throws EvaluationException {
        Date date = testsHelper.getDate("2018-04-10 00:00:00.0");
        PatientCalculationContext calculationContext = patientCalculationService.createCalculationContext();
        EvaluationContext context = new EvaluationContext();
        context.addParameterValue("onOrBefore", date);
        ...
        when(patientSetService.getAllPatients()).thenReturn(patients);
        evaluator.evaluate(definition, context);
        verify(patientCalculationService).evaluate(patients.getMemberIds(), definition.getCalculation(), definition.getCalculationParameters(), calculationContext);
    }
