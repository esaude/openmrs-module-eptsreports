...
public class CalculationsTestsCache {
	...
	void stubEptsCalculations(final PatientCalculation calculation) {
		new MockUp<EptsCalculations>() {
			...
			@Mock
			public CalculationResultMap lastObs(List<EncounterType> encounterTypes, Concept concept, Location location,
				...
				for (int i : cohort) {
					...
					if (p != null) {
						Obs matchedObs = null;
						List<Obs> observations = Context.getObsService().getObservationsByPersonAndConcept(p, concept);
						// from last to first
			...
			private void addObsToMap(final PatientCalculation calculation, PatientCalculationContext context,
			        CalculationResultMap map, int i, Obs matchedObs) {
				if (matchedObs != null) {
	...
	void stubEptsCalculationUtils(final PatientCalculation calculation) {
		new MockUp<EptsCalculationUtils>() {
			...
			public Obs obsResultForPatient(CalculationResultMap results, Integer patientId) {
				return (results == null || results.isEmpty() || !results.containsKey(patientId) || results.get(patientId) == null) ? null
