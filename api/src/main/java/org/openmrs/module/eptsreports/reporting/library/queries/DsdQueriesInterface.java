  ...
  class QUERY {
    ...
    public static final String findPatientsInArtWhoAreStable =
        "select patient_id from( select inicio.patient_id, inicio.data_inicio, timestampdiff(year,per.birthdate,:endDate) idade, timestampdiff(month,inicio.data_inicio,:endDate) idadeEmTarv, cd4Absoluto.value_numeric cd4Abs, cd4Percentual.value_numeric cd4Per, cvmenor1000.patient_id pidcv12meses,cvmenor1000.person_id pidcvmenor100  from ( "
            ...
            + "union "
            + "Select p.patient_id,o.value_numeric from patient p "
            + "inner join encounter e on p.patient_id=e.patient_id  "
            + "inner join obs o on e.encounter_id=o.encounter_id "
            + "inner join obs obsData on e.encounter_id=obsData.encounter_id "
            + "where p.voided=0 and e.voided=0 and o.voided=0 and o.concept_id=23896 and e.encounter_type=53 and obsData.voided=0 and obsData.concept_id=1190 and "
            + "obsData.value_datetime between (:endDate - INTERVAL 12 MONTH) AND :endDate and e.location_id=:location and o.value_numeric>200 "
            ...
            + "group by p.patient_id "
            + ") max_cd4 "
            + "left join obs o on o.person_id=max_cd4.patient_id and max_cd4.max_data_cd4=o.obs_datetime and o.voided=0 and  o.concept_id=730 and o.value_numeric>15 and o.location_id=:location "
            + "union "
            + "select p.patient_id,o.value_numeric from patient p "
            + "inner join encounter e on p.patient_id=e.patient_id  "
            + "inner join obs o on e.encounter_id=o.encounter_id "
            + "inner join obs obsData on e.encounter_id=obsData.encounter_id "
            + "where p.voided=0 and e.voided=0 and o.voided=0 and o.concept_id=730 and e.encounter_type=53 and obsData.voided=0 and obsData.concept_id=1190 and "
            + "obsData.value_datetime between (:endDate - INTERVAL 12 MONTH) AND :endDate and e.location_id=:location and o.value_numeric>15 "
            + ") cd4Percentual "
            + "on inicio.patient_id=cd4Percentual.patient_id "
            + ") elegivel "
