  ...
  public static String getPatientHavingLastScheduledDrugPickupDate(
    ...
    String query =
        "SELECT final.patient_id   "
            + "             from(   "
            + "                SELECT   "
            + "                    most_recent.patient_id, Date_add(Max(o.value_datetime), interval 30 day) final_encounter_date  "
            + "                      "
            + "                FROM   (SELECT enc.patient_id,   "
            + "                                Max(enc.encounter_datetime)  encounter_datetime , max(enc.encounter_id)  as encounter_id  "
            + "                        FROM   patient pa   "
            ...
            + "                        UNION   "
            + "                        SELECT enc.patient_id,   "
            + "                                Date_add(Max(obs.value_datetime) , interval 30 day) encounter_datetime  , max(enc.encounter_id)  as encounter_id "
            + "                        FROM   patient pa   "
            + "                                inner join encounter enc   "
            + "                                        ON enc.patient_id =   "
            + "                                            pa.patient_id   "
            ...
            + "                            e.patient_id = most_recent.patient_id  and e.encounter_type  in (%s,%s,%s,%s)  "
            + "                            and e.location_id=   :location and e.voided=0  "
            + "                            and e.encounter_id=most_recent.encounter_id  and e.encounter_datetime <   :onOrBefore  "
            + "                        inner join obs o  on   "
