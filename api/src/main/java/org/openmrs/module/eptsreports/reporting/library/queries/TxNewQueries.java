...
public class TxNewQueries {
	...
	/**
	 * TxNew Queries Looks for patients enrolled in ART program (program
	 * 2=SERVICO TARV - TRATAMENTO) before or on end date (Parameters:
	 * getARTProgram) UNION Looks for patients registered as START DRUGS (answer
	 * to question 1255 = ARV PLAN is 1256 = START DRUGS) in the first drug
	 * pickup (encounter type 18=S.TARV: FARMACIA) or follow up consultation for
	 * adults and children (encounter types 6=S.TARV: ADULTO SEGUIMENTO and
	 * 9=S.TARV: PEDIATRIA SEGUIMENTO) before or on end date (Parameters:
	 * getARVPharmaciaEncounterType, getAdultoSeguimentoEncounterType,
	 * getARVPediatriaSeguimentoEncounterType, getARVPlanConcept,
	 * getstartDrugsConcept) UNION Looks for with START DATE (Concept
	 * 1190=HISTORICAL DRUG START DATE) filled in drug pickup (encounter type
	 ...
	 * SEGUIMENTO) where START DATE is before or equal end date (Parameters: )
	 * UNION Looks for patients with first drug pickup (encounter type
	...
	public static String getTxNewUnionQueries(Map<String, Integer> parameters) {
		...
		if (parameters.get("minAge") != null) {
			minAgeCondition = "timestampdiff(year,birthdate,data_inicio)>="
					+ parameters.get("minAge");
		}

		if (parameters.get("maxAge") != null) {
			maxAgeCondition = "timestampdiff(year,birthdate,data_inicio)<"
					+ parameters.get("maxAge");
		}
		...
		String finalQuery = "select patient_id from (select patient_id, min(data_inicio) data_inicio from "
				...
				+ ","
				+ parameters.get("arvAdultoSeguimentoEncounter")
				+ ","
				+ parameters.get("arvPediatriaSeguimentoEncounter")
				+ ") and o.concept_id="
				+ parameters.get("arvPlanConcept")
				+ " and o.value_coded="
				...
				+ parameters.get("arvPediatriaSeguimentoEncounter")
				+ ") and o.concept_id="
				+ parameters.get("historicalDrugsConcept")
				...
				+ "where (data_inicio between :onOrAfter and :onOrBefore) and "
				+ ageClause;
