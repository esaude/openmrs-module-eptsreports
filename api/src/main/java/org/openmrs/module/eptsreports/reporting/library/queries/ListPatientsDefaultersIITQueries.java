  ...
  class QUERY {
    ...
    public static final String findPatientsDefaultersIITList =
        " select distinct inicio.patient_id,                         																		"
            ...
            + "     datediff(:endDate,iit_patients.encounter_datetime) diasFalta,                         																							"
            + "     estadoMulher.estadoMulher as 'estadoMulher',                         																											"
            + "     tb.temTB as 'temTB',                         																																	"
            + "     consentimentoPaciente.consente as 'consentimentoPaciente',                         																								"
            + "     consentimentoConfidente.consente as 'consentimentoConfidente',                         																							"
            + "     dispensaTipo.tipoDispensa,                         																																"
            + "     consultaClinica.encounter_datetime as 'ultimaConsulta',                         																								"
            + "      consultaClinica.value_datetime as 'proximaConsulta',                           																								"
            + "     max_fila.last_fila_pickup as 'ultimoFila',                         																												"
            + "     max_fila.next_scheduled_fila_pickup as 'proximoLevantamentoFila',                         																						"
            + "     max_recepcao.data_recepcao_levantou as 'ultimoLevantamentoMCard',                         																						"
            + "     DATE_ADD(max_recepcao.data_recepcao_levantou, INTERVAL 30 DAY) as 'proximoLevantamentoMCard'                           															"
            + " from                         																																						"
            + " (                         																																							"
            ...
            + "               where e.voided=0 and p.voided=0 and obsObito.voided=0 and e.encounter_type in (21,36,37) and  e.encounter_datetime<=:endDate and  e.location_id=:location             "
            ...
            + "                   inner join encounter e on p.patient_id=e.patient_id                                                              													"
            + "                   inner join obs o on o.encounter_id=e.encounter_id                                                              													"
            + "               where e.voided=0 and p.voided=0 and e.encounter_datetime<=:endDate and o.voided=0 and o.concept_id=2016                          										"
            ...
            + "     where all_consulta_levantamento.encounter_datetime <= all_saidas.data_estado and all_saidas.data_estado <= :endDate                         									"
            + "                            																																							"
            ...
            + "           inner join obs next_fila on next_fila.encounter_id = e.encounter_id                         																				"
            ...
            + "                 inner join encounter e on p.patient_id=e.patient_id                          																						"
            + "                 inner join obs data_levantamento_arv on data_levantamento_arv.encounter_id = e.encounter_id                          												"
            ...
            + " inner join person p on inicio.patient_id = p.person_id                         																										"
            + " left join                          																																					"
            + " ( select pad1.*                         																																			"
            ...
            + " ) pad3 on pad3.person_id=iit_patients.patient_id                                 																									"
            + " left join                                																																			"
            + " ( select pn1.*                         																																				"
            ...
            + " ) pn on pn.person_id=iit_patients.patient_id                               																											"
            + " left join                         																																					"
            + " (   select pid1.*                         																																			"
            ...
            + " ) pid on  iit_patients.patient_id= pid.patient_id                         																											"
            + " left join person_attribute pat on pat.person_id=iit_patients.patient_id and pat.person_attribute_type_id=9 and pat.value is not null and pat.value<>'' and pat.voided=0             "
            + "  left join                          																																				"
            + " (                         																																							"
            ...
            + " ) estadoMulher on iit_patients.patient_id = estadoMulher.patient_id                         																						"
            + " left join                         																																					"
            + " (                         																																							"
            + "     SELECT  p.patient_id,'Sim' temTB                          																														"
            + "   FROM  patient p                         																																			"
            + "       INNER JOIN encounter e ON e.patient_id = p.patient_id                         																								"
            + "       INNER JOIN obs o ON o.encounter_id = e.encounter_id                         																									"
            ...
            + "     GROUP BY p.patient_id                         																																	"
            + " ) tb on iit_patients.patient_id= tb.patient_id                         																												"
            + " left join                          																																					"
            + " (                         																																							"
            + "   select maxConsent.patient_id,if(o.value_coded=1065,'Sim','Não') consente                         																					"
            + "   from                          																																					"
            + "   (                         																																						"
            ...
            + "       o.concept_id=6306                          																																	"
            + " ) consentimentoPaciente on iit_patients.patient_id = consentimentoPaciente.patient_id                         																		"
            + " left join                          																																					"
            + " (                         																																							"
            + "   select maxConsent.patient_id,if(o.value_coded=1065,'Sim','Não') consente                         																					"
            + "   from                          																																					"
            ...
            + "       e.encounter_datetime=maxConsent.dataConsentimento and                          																								"
            + "       o.concept_id=6177                          																																	"
            + " ) consentimentoConfidente on iit_patients.patient_id = consentimentoConfidente.patient_id                         																	"
            + " left join                          																																					"
            + " (                         																																							"
            ...
            + "   )dispensafinal                         																																			"
            + "   group by patient_id                         																																		"
            + " ) dispensaTipo on iit_patients.patient_id = dispensaTipo.patient_id                         																						"
            + " left join                          																																					"
            + " (                         																																							"
            ...
            + " ) consultaClinica on iit_patients.patient_id = consultaClinica.patient_id                         																					"
            + " left join                         																																					"
            + " ( select max_fila.patient_id, max_fila.encounter_datetime last_fila_pickup, next_scheduled_fila.value_datetime next_scheduled_fila_pickup from                          			"
            + " (                         																																							"
            + "   select p.patient_id,max(e.encounter_datetime) encounter_datetime from patient p                          																			"
            + "     inner join encounter e on e.patient_id=p.patient_id                           																									"
            + "   where p.voided=0 and e.voided=0 and e.encounter_type = 18 and e.encounter_datetime<=:endDate and e.location_id=:location                         									"
            + "     group by p.patient_id                         																																	"
            + " ) max_fila                         																																					"
            + "   inner join encounter e on e.patient_id = max_fila.patient_id and e.encounter_datetime = max_fila.encounter_datetime                         										"
            + "   left join obs next_scheduled_fila on next_scheduled_fila.encounter_id = e.encounter_id                         																	"
            + " where  e.voided = 0 and next_scheduled_fila.voided = 0 and e.encounter_type =18                          																			"
            + "   and next_scheduled_fila.concept_id = 5096 and next_scheduled_fila.value_datetime is not null                         																"
            + " ) max_fila on iit_patients.patient_id=max_fila.patient_id                          																									"
            + " left join                         																																					"
            + " (                         																																							"
            ...
            + "   group by p.patient_id                                                         																									"
            + " ) max_recepcao on iit_patients.patient_id=max_recepcao.patient_id           																										";

    public static final String findPatientsDefaultersIIT =
        "  select distinct inicio.patient_id from                                                                                                "
            + " (                                                                                                                                      												"
            + "   select art_started.patient_id, art_started .art_start_date from                                                                      												"
            + "   (                                                                                                                                    												"
            + "       select patient_id, art_start_date from                                                                                           												"
