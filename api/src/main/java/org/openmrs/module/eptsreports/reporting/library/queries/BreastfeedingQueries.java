...
public class BreastfeedingQueries {
	 ...
	 */
	public static String getPregnantWhileOnArt(int concept1, int concept2, int concept3, int concept4, int enc1, int enc2,
	        int program) {
		
		return "Select 	p.patient_id" + " from 	patient p" + " inner join encounter e on p.patient_id=e.patient_id"
		        + " inner join obs o on e.encounter_id=o.encounter_id"
		        + " where p.voided=0 and e.voided=0 and o.voided=0 and concept_id=" + concept1 + " and value_coded="
		        + concept2 + " and e.encounter_type in (" + enc1 + "," + enc2
		        ...
		        + " inner join obs o on e.encounter_id=o.encounter_id"
		        + " where 	p.voided=0 and e.voided=0 and o.voided=0 and concept_id=" + concept3 + " and"
		        + " e.encounter_type in (" + enc1 + "," + enc2
		        + ") and e.encounter_datetime between :startDate and :endDate and e.location_id=:location" + " union"
		        + " Select p.patient_id" + " from patient p inner join encounter e on p.patient_id=e.patient_id"
		        + " inner join obs o on e.encounter_id=o.encounter_id"
		        + " where p.voided=0 and e.voided=0 and o.voided=0 and concept_id=" + concept4 + " and"
		        + " e.encounter_type in (" + enc1 + "," + enc2
		        + ") and e.encounter_datetime between :startDate and :endDate and e.location_id=:location" + " union"
		        + " select pp.patient_id from patient_program pp" + " where pp.program_id=" + program
