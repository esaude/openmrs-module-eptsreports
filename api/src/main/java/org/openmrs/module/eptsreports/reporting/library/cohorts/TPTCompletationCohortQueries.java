...
public class TPTCompletationCohortQueries {
	...
	public CohortDefinition findTxCurrWithTPTCompletation() {
		...
		dsd.addSearch("TXCURR",
				EptsReportUtils.map(this.txCurrCohortQueries.findPatientsWhoAreActiveOnART(), mappings));
		dsd.addSearch("STARTED-INH",
				EptsReportUtils.map(this.findPatientsWhoStartedINHTherapyBeforeReportingEndDate(), mappings));
		dsd.addSearch("COMPLETED-INH", EptsReportUtils.map(this.findPatientsWhoCompletedINHTherapy(), mappings));
		dsd.addSearch("STARTED-3HP",
				EptsReportUtils.map(this.findPatientsWhoStarted3HPTherapyBeforeReportEndDate(), mappings));
		dsd.addSearch("COMPLETED-3HP",
				EptsReportUtils.map(this.findPatientsWhoCompleted3HPTherapyBeforeReportEndDate(), mappings));

		dsd.setCompositionString("TXCURR AND ((STARTED-INH AND COMPLETED-INH) OR (STARTED-3HP AND COMPLETED-3HP))");
	...
	public CohortDefinition findTxCurrWithoutTPTCompletation() {
		...
		dsd.addSearch("TXCURR",
				EptsReportUtils.map(this.txCurrCohortQueries.findPatientsWhoAreActiveOnART(), mappings));
		dsd.addSearch("TPT-COMPLETATION", EptsReportUtils.map(this.findTxCurrWithTPTCompletation(), mappings));
	...
	public CohortDefinition findTxCurrWithoutTPTCompletionWhoWereTreatedForTBForLast3Years() {
		...
		dsd.addSearch("TPT-NO-COMPLETION", EptsReportUtils.map(this.findTxCurrWithoutTPTCompletation(), mappings));
	...
	public CohortDefinition findTxCurrWithoutTPTCompletionWithPositivTBScreening() {
		...
		dsd.addSearch("TB-POSITIVE-SCREENING",
	...
	public CohortDefinition findTxCurrWithoutTPTCompletionButEligibleForTPTCompletation() {
		...
		dsd.addSearch("TPT-NO-COMPLETION-WITH-TB",
				EptsReportUtils.map(this.findTxCurrWithoutTPTCompletionWhoWereTreatedForTBForLast3Years(), mappings));

		dsd.addSearch("TPT-NO-COMPLETION-WITH-TB-POSITIVE-SCREENING",
	...
	public CohortDefinition findTxCurrWithoutTPTCompletionWhoInitiatedTPTInLast7Months() {
		...
		dsd.addSearch("TPT-NO-COMPLETION", EptsReportUtils.map(this.findTxCurrWithoutTPTCompletation(), mappings));
		dsd.addSearch("TBPREV-DENOMINATOR", EptsReportUtils.map(this.getTbPrevTotalDenominator(), mappings));
	...
	public CohortDefinition findTxCurrWithoutTPTCompletionButEligibleForTPTInitiation() {
		...
		dsd.addSearch("TXCURR-ELIGIBLE-TPT-COMPLETION",
				EptsReportUtils.map(this.findTxCurrWithoutTPTCompletionButEligibleForTPTCompletation(), mappings));

		dsd.addSearch("TXCURR-TPT-INITIATION",
				EptsReportUtils.map(this.findTxCurrWithoutTPTCompletionWhoInitiatedTPTInLast7Months(), mappings));
	...
	private CohortDefinition findPatientsWhoStartedINHTherapyBeforeReportingEndDate() {
		...
		definition.setQuery(TPTCompletationQueries.QUERY.findPatientsWhoStartedINHTherapyBeforeReportingEndDate);
	...
	private CohortDefinition findPatientsWhoStarted3HPTherapyBeforeReportEndDate() {
		...
		dsd.addSearch("STARTED-3HP",
				EptsReportUtils.map(
						this.genericCohorts.generalSql("Patients Who have Started 3HP Therapy before Report End Date",
								TPTCompletationQueries.QUERY.findPatientsWhoStarted3HPTherapyBeforeReportingEndDate),
						mappings));
	...
	private CohortDefinition findPatientsWhoCompleted3HPTherapyBeforeReportEndDate() {
		...
		dsd.addSearch("COMPLETED-3HP",
				EptsReportUtils.map(this.genericCohorts.generalSql("get Patients Who Completed 3HP Therapy",
						TPTCompletationQueries.QUERY.findPatientsWhoCompleted3HPTherapy), mappings));
		dsd.addSearch("COMPLETED-INH", EptsReportUtils.map(this.findPatientsWhoCompletedINHTherapy(), mappings));
	...
	private CohortDefinition getTxTBNumerator() {
		...
		cd.addSearch("A", EptsReportUtils.map(A, "startDate=${endDate-1095d},endDate=${endDate},location=${location}"));

		cd.addSearch("started-tb-treatment-previous-period",
				EptsReportUtils.map(tXTBCohortQueries.getTbDrugTreatmentStartDateWithinReportingDate(),
	...
	private CohortDefinition generateTxTBNumerator() {
		final CompositionCohortDefinition cd = new CompositionCohortDefinition();
		String generalParameterMapping = "startDate=${endDate-1095d},endDate=${endDate},location=${location}";

		cd.setName("TxTB - Numerator - generating Compositions");
		final CohortDefinition i = this.genericCohorts
				.generalSql("onTbTreatment",
						TXTBQueries
								.dateObs(this.tbMetadata.getTBDrugTreatmentStartDate().getConceptId(),
										Arrays.asList(this.hivMetadata.getAdultoSeguimentoEncounterType().getId(),
		...
		cd.addSearch("iii",
				EptsReportUtils.map(tXTBCohortQueries.getPulmonaryTBWithinReportingDate(), generalParameterMapping));
		cd.addSearch("iv", EptsReportUtils.map(tXTBCohortQueries.getTuberculosisTreatmentPlanWithinReportingDate(),
	...
	private CohortDefinition getTxTBDenominatorAndPositiveScreening() {
		...
		String generalParameterMapping = "startDate=${endDate-14d},endDate=${endDate},location=${location}";

		definition.setName("TxTB - Denominator and Positive Sreening");
		definition.addSearch("denominator",
				EptsReportUtils.map(
						this.getTxTBDenominator(generalParameterMapping,
								"startDate=${endDate-6m-14d},endDate=${endDate-14d-1d},location=${location}"),
						generalParameterMapping));
		definition.addSearch("positive-screening",
				EptsReportUtils.map(this.getTxTBPPositiveScreening(generalParameterMapping), generalParameterMapping));
	...
	private CohortDefinition getTxTBPPositiveScreening(String generalParameterMapping) {
		...
		cd.addSearch("A", EptsReportUtils.map(tXTBCohortQueries.codedYesTbScreening(),
				"onOrAfter=${endDate-14d},onOrBefore=${endDate},locationList=${location}"));
		cd.addSearch("B", EptsReportUtils.map(tXTBCohortQueries.positiveInvestigationResultComposition(),
				generalParameterMapping));
		cd.addSearch("C",
				EptsReportUtils.map(
						tXTBCohortQueries.negativeInvestigationResultAndAnyResultForTBScreeningComposition(),
						generalParameterMapping));
		cd.addSearch("D", EptsReportUtils.map(tXTBCohortQueries.getTbDrugTreatmentStartDateWithinReportingDate(),
				generalParameterMapping));
		cd.addSearch("E", EptsReportUtils.map(tXTBCohortQueries.getInTBProgram(), generalParameterMapping));
		cd.addSearch("F",
				EptsReportUtils.map(tXTBCohortQueries.getPulmonaryTBWithinReportingDate(), generalParameterMapping));
		cd.addSearch("G", EptsReportUtils.map(tXTBCohortQueries.getTuberculosisTreatmentPlanWithinReportingDate(),
				generalParameterMapping));
		cd.addSearch("H", EptsReportUtils.map(tXTBCohortQueries.getAllTBSymptomsForDisaggregationComposition(),
				generalParameterMapping));
		cd.addSearch("I", EptsReportUtils.map(tXTBCohortQueries.getSputumForAcidFastBacilliWithinReportingDate(),
				generalParameterMapping));
	...
	private CohortDefinition getTxTBDenominator(String generalParameterMapping, String specificParameterMapping) {

		final CompositionCohortDefinition definition = new CompositionCohortDefinition();
		this.addGeneralParameters(definition);
		definition.setName("TxTB - Denominator");
		definition.addSearch("art-list", EptsReportUtils.map(this.genericCohorts.getStartedArtBeforeDate(false),
				"onOrBefore=${endDate},location=${location}"));
		definition.addSearch("tb-screening",
				EptsReportUtils.map(tXTBCohortQueries.yesOrNoInvestigationResult(), generalParameterMapping));
		definition.addSearch("tb-investigation", EptsReportUtils
				.map(tXTBCohortQueries.positiveInvestigationResultComposition(), generalParameterMapping));
		definition.addSearch("started-tb-treatment", EptsReportUtils
				.map(tXTBCohortQueries.getTbDrugTreatmentStartDateWithinReportingDate(), generalParameterMapping));
		definition.addSearch("in-tb-program",
				EptsReportUtils.map(tXTBCohortQueries.getInTBProgram(), generalParameterMapping));

		definition.addSearch("started-tb-treatment-previous-period", EptsReportUtils
				.map(tXTBCohortQueries.getTbDrugTreatmentStartDateWithinReportingDate(), specificParameterMapping));
		definition.addSearch("in-tb-program-previous-period",
				EptsReportUtils.map(tXTBCohortQueries.getInTBProgram(), specificParameterMapping));

		definition.addSearch("transferred-out",
				EptsReportUtils.map(tXTBCohortQueries.getPatientsWhoAreTransferredOut(), generalParameterMapping));

		CohortDefinition fichaResumoMasterCard = this.genericCohorts.generalSql("onFichaResumoMasterCard",
				TXTBQueries.dateObsByObsDateTimeClausule(this.tbMetadata.getPulmonaryTB().getConceptId(),
						this.hivMetadata.getYesConcept().getConceptId(),
						this.hivMetadata.getMasterCardEncounterType().getEncounterTypeId()));

		CohortDefinition fichaClinicaMasterCard = this.genericCohorts.generalSql("fichaClinicaMasterCard",
		...
		definition.addSearch("ficha-resumo-master-card",
				EptsReportUtils.map(fichaResumoMasterCard, generalParameterMapping));
		definition.addSearch("ficha-clinica-master-card",
				EptsReportUtils.map(fichaClinicaMasterCard, generalParameterMapping));
		definition.addSearch("all-tb-symptoms",
				EptsReportUtils.map(this.getAllTBSymptomsForDemoninatorComposition(), generalParameterMapping));

		definition.setCompositionString("(art-list AND "
	...
	private CohortDefinition getAllTBSymptomsForDemoninatorComposition() {
		...
		definition.addSearch("tuberculosis-symptoms",
				EptsReportUtils
						.map(tXTBCohortQueries.getTuberculosisSymptoms(this.hivMetadata.getYesConcept().getConceptId(),
								this.hivMetadata.getNoConcept().getConceptId()), generalParameterMapping));

		definition.addSearch("active-tuberculosis",
				EptsReportUtils.map(tXTBCohortQueries.getActiveTuberculosis(), generalParameterMapping));

		definition.addSearch("tb-observations",
				EptsReportUtils.map(tXTBCohortQueries.getTbObservations(), generalParameterMapping));

		definition.addSearch("application-for-laboratory-research",
				EptsReportUtils.map(tXTBCohortQueries.getApplicationForLaboratoryResearch(), generalParameterMapping));

		definition.addSearch("tb-genexpert-or-culture-test-or-lam-test", EptsReportUtils
				.map(tXTBCohortQueries.getTbGenExpertORCultureTestOrTbLamOrBk(), generalParameterMapping));
	...
	public CohortDefinition getTbPrevTotalDenominator() {
		...
		dsd.addSearch("STARTED-TPT", EptsReportUtils.map(this.genericCohorts.generalSql(
				...
				TPTCompletationQueries.QUERY.findPatientsWhoStartedTbPrevPreventiveTreatmentDuringPreviousReportingPeriod),
				mappings));
		dsd.addSearch("TRF-OUT", EptsReportUtils.map(txTbPrevCohortQueries.findPatientsTransferredOut(), mappings));
		dsd.addSearch("ENDED-TPT", EptsReportUtils.map(this.genericCohorts.generalSql(
				"Finding Patients Who have Completed TPT",
				TPTCompletationQueries.QUERY.findPatientsWhoCompletedTbPrevPreventiveTreatmentDuringReportingPeriod),
				mappings));
...
//        "(STARTED-TPT AND (NEWLY-ART OR PREVIOUS-ART)) NOT (TRF-OUT NOT ENDED-TPT) ");
	...
	public CohortDefinition findPatientsWhoCompletedINHTherapy() {
		...
		dsd.addSearch("INH-1", EptsReportUtils.map(findPatientsWhoCompletedINHTherapyQuery1(), mappings));

		dsd.addSearch("INH-2", EptsReportUtils.map(findPatientsWhoCompletedINHTherapyQuery2(), mappings));

		dsd.addSearch("INH-3", EptsReportUtils.map(findPatientsWhoCompletedINHTherapyQuery3(), mappings));

		dsd.addSearch("INH-4", EptsReportUtils.map(findPatientsWhoCompletedINHTherapyQuery4(), mappings));

		dsd.addSearch("INH-5", EptsReportUtils.map(findPatientsWhoCompletedINHTherapyQuery5(), mappings));
