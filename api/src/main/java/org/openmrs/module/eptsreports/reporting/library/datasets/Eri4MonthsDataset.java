...
public class Eri4MonthsDataset extends BaseDataSet {
	...
	public DataSetDefinition constructEri4MonthsDataset() {
		...
		String cohortPeriodMappings = "cohortStartDate=${endDate-5m+1d},cohortEndDate=${endDate-4m},location=${location}";
		String dimensionMappings = cohortPeriodMappings + ",reportingStartDate=${startDate},reportingEndDate=${endDate}";
		String reportingPeriodMappings = "startDate=${startDate},endDate=${endDate},location=${location}";
		dsd.setName("ERI-4months Data Set");
		dsd.addParameters(getParameters());
		
		// apply disaggregations here
		dsd.addDimension("state", EptsReportUtils.map(eptsCommonDimension.getEri4MonthsDimension(), dimensionMappings));
		...
		addRow(dsd, "I1", "All Patients retained on ART 4 months after ART initiation", EptsReportUtils.map(
		    eptsGeneralIndicator.getIndicator("all patients", EptsReportUtils.map(
		        eri4MonthsCohortQueries.getPatientsWhoInitiatedArtLessTransferIns(), cohortPeriodMappings)),
		    reportingPeriodMappings), get4MonthsRetentionColumns());
		addRow(dsd, "I2", "Pregnant women retained on ART 4 months after ART initiation", EptsReportUtils.map(
		    eptsGeneralIndicator.getIndicator("pregnant women", EptsReportUtils.map(
		        eri4MonthsCohortQueries.getPregnantWomenRetainedOnArtFor4MonthsFromArtInitiation(), cohortPeriodMappings)),
		    reportingPeriodMappings), get4MonthsRetentionColumns());
		addRow(dsd, "I3", "Breastfeeding women retained on ART 3 months after ART initiation", EptsReportUtils.map(
		    eptsGeneralIndicator.getIndicator("breastfeeding women", EptsReportUtils.map(
		        ...
		        cohortPeriodMappings)), reportingPeriodMappings), get4MonthsRetentionColumns());
		addRow(dsd, "I4",
		    ...
		    EptsReportUtils.map(
		        eptsGeneralIndicator.getIndicator("children", EptsReportUtils.map(
		            eri4MonthsCohortQueries.getChildrenRetainedOnArtFor4MonthsFromArtInitiation(), cohortPeriodMappings)),
		        reportingPeriodMappings), get4MonthsRetentionColumns());
		addRow(dsd, "I5",
		    ...
		    EptsReportUtils.map(
		        eptsGeneralIndicator.getIndicator("adults", EptsReportUtils.map(
		            eri4MonthsCohortQueries.getAdultsRetaineOnArtFor4MonthsFromArtInitiation(), cohortPeriodMappings)),
		        reportingPeriodMappings), get4MonthsRetentionColumns());
		return dsd;
	}
	...
	private List<ColumnParameters> get4MonthsRetentionColumns() {
		...
		ColumnParameters aliveNotInTreatment = new ColumnParameters("Not in treatment", "Alive and NOT in treatment",
		        "state=ANIT", "07");
		return Arrays.asList(initiatedArt, aliveInTreatment, dead, lostTfu, transfers, stopped, aliveNotInTreatment);
	}
}
