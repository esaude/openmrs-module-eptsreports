  ...
  public DataDefinition getNumberOfDaysOfDelay() {
    ...
    String query =
        " SELECT DISTINCT p.patient_id,                                                                                                                                                             "
            ...
            + "           WHEN levantamento.patient_id IS NOT NULL THEN  TIMESTAMPDIFF(DAY, levantamento.last_pickup,:endDate) "
            + "           WHEN fila.patient_id IS NOT NULL THEN  TIMESTAMPDIFF(DAY, fila.last_pickup,:endDate) "
            + "           END  "
            + "   FROM patient p "
            + "   LEFT JOIN (SELECT p.patient_id, Max(e.encounter_datetime) AS last_pickup "
            + "      FROM   patient p  "
            + "                 INNER JOIN encounter e ON p.patient_id = e.patient_id  "
            + "      WHERE  p.voided = 0 "
            + "        AND e.voided = 0  "
            ...
            + "                                   AND (o2.concept_id = ${23865} AND o2.value_coded = ${1065})  "
            + "                                 GROUP  BY p.patient_id)  "
            + "      GROUP  BY p.patient_id) AS levantamento ON levantamento.patient_id = p.patient_id "
            + "   LEFT JOIN(SELECT p.patient_id, Max(e.encounter_datetime) AS last_pickup  "
            + "     FROM   patient p "
            + "                INNER JOIN encounter e ON p.patient_id =  e.patient_id  "
            + "                INNER JOIN obs o1 ON e.encounter_id = o1.encounter_id "
            ...
            + "           AND e.encounter_datetime <= :endDate "
            + "         GROUP  BY p.patient_id)  "
            + "     GROUP  BY p.patient_id) AS fila ON fila.patient_id = p.patient_id  "
            + "   LEFT JOIN(SELECT p.patient_id, Max(e.encounter_datetime) AS last_pickup  "
            + "   FROM   patient p   "
            + "       INNER JOIN encounter e ON p.patient_id = e.patient_id  "
            + "   WHERE  p.voided = 0  "
            ...
            + "   LEFT JOIN (SELECT p.patient_id, Max(o1.value_datetime) AS last_pickup  "
            + "   FROM   patient p   "
            + "       INNER JOIN encounter e ON p.patient_id =  e.patient_id   "
            + "       INNER JOIN obs o1 ON e.encounter_id = o1.encounter_id  "
            + "       INNER JOIN obs o2 ON e.encounter_id = o2.encounter_id  "
            ...
            + "   LEFT JOIN (SELECT p.patient_id, DATE_ADD(Max(o.value_datetime), interval 30 DAY) AS last_scheduled "
            + "   FROM   patient p   "
            + "       INNER JOIN encounter e ON p.patient_id =  e.patient_id   "
            + "       INNER JOIN obs o ON e.encounter_id = o.encounter_id  "
            + "       INNER JOIN obs o2 ON e.encounter_id = o2.encounter_id  "
