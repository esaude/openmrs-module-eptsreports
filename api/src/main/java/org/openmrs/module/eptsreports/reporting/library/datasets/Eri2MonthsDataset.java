...
public class Eri2MonthsDataset extends BaseDataSet {
	...
	public DataSetDefinition constructEri2MonthsDatset() {
		...
		dsd.addDimension("state", EptsReportUtils.map(eptsCommonDimension.getEri2MonthsDimension(), mappingsInd));
		...
		addRow(
		    ...
		    EptsReportUtils.map(
		        eptsGeneralIndicator.getIndicator("All patients",
		            EptsReportUtils.map(eri2MonthsCohortQueries.getAllPatientsWhoInitiatedArt(), mappingsInd)), mappings),
		    get2MonthsRetentionColumns());
		addRow(dsd, "R22", "Pregnant women retained on ART 2 months after ART initiation",
		    EptsReportUtils.map(
		        eptsGeneralIndicator.getIndicator("Pregnant women", EptsReportUtils.map(
		            eri2MonthsCohortQueries.getPregnantWomenRetainedOnArtFor2MonthsFromArtInitiation(), mappingsInd)),
		        mappings), get2MonthsRetentionColumns());
		addRow(dsd, "R23", "Breastfeeding women retained on ART 2 months after ART initiation", EptsReportUtils.map(
		    eptsGeneralIndicator.getIndicator("Breastfeeding women", EptsReportUtils.map(
		        eri2MonthsCohortQueries.getBreastfeedingWomenRetainedOnArtFor2MonthsFromArtInitiation(), mappingsInd)),
		    mappings), get2MonthsRetentionColumns());
		addRow(
		    dsd,
		    "R24",
		    ...
		    EptsReportUtils.map(
		        eptsGeneralIndicator.getIndicator("Children", EptsReportUtils.map(
		            eri2MonthsCohortQueries.getChildrenRetainedOnArtFor2MonthsFromArtInitiation(), mappingsInd)), mappings),
		    get2MonthsRetentionColumns());
		addRow(
		    dsd,
		    "R25",
		    ...
		    EptsReportUtils.map(
		        eptsGeneralIndicator.getIndicator("Adults", EptsReportUtils.map(
		            eri2MonthsCohortQueries.getAdultsRetainedOnArtFor2MonthsFromArtInitiation(), mappingsInd)), mappings),
		    get2MonthsRetentionColumns());
		return dsd;
	}
	
	private List<ColumnParameters> get2MonthsRetentionColumns() {
		ColumnParameters artStart = new ColumnParameters("initiated ART", "Initiated ART", "state=IART", "01");
		ColumnParameters didNotPickUpDrugs = new ColumnParameters("did Not pick drugs", "Did Not pick up drugs",
		        "state=DNPUD", "02");
		ColumnParameters pickedUpDrugs = new ColumnParameters("picked drugs", "Picked up Drugs", "state=PUD", "03");
