...
public class Eri2MonthsDataset extends BaseDataSet {
	...
	public DataSetDefinition constructEri2MonthsDatset() {
		...
		dsd.addDimension("state", EptsReportUtils.map(eptsCommonDimension.getEri2MonthsDimension(), mappings));
		...
		addRow(
				...
				EptsReportUtils.map(
						eptsGeneralIndicator.getIndicator("All patients", EptsReportUtils.map(
								eri2MonthsCohortQueries.getPatientsRetainedOnArtFor2MonthsFromArtInitiation(), mappings)), mappings),
				get2MonthsRetentionColumns());
		addRow(dsd, "R22", "Pregnant women retained on ART 2 months after ART initiation", EptsReportUtils.map(
				eptsGeneralIndicator.getIndicator("Pregnant women", EptsReportUtils.map(
						eri2MonthsCohortQueries.getPregnantWomenRetainedOnArtFor2MonthsFromArtInitiation(), mappings)), mappings),
				get2MonthsRetentionColumns());
		addRow(dsd, "R23", "Breastfeeding women retained on ART 2 months after ART initiation", EptsReportUtils.map(
				eptsGeneralIndicator.getIndicator("Breastfeeding women", EptsReportUtils.map(
						eri2MonthsCohortQueries.getBreastfeedingWomenRetainedOnArtFor2MonthsFromArtInitiation(), mappings)),
		...
		addRow(
				...
				EptsReportUtils.map(
						eptsGeneralIndicator.getIndicator("Children", EptsReportUtils.map(
								eri2MonthsCohortQueries.getChildrenRetainedOnArtFor2MonthsFromArtInitiation(), mappings)), mappings),
				get2MonthsRetentionColumns());
		addRow(
				dsd,
				"R25",
				...
				EptsReportUtils.map(
						eptsGeneralIndicator.getIndicator("Adults", EptsReportUtils.map(
								eri2MonthsCohortQueries.getAdultsRetaineOnArtFor2MonthsFromArtInitiation(), mappings)), mappings),
				get2MonthsRetentionColumns());
		return dsd;
