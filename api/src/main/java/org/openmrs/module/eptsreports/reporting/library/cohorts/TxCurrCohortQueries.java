...
public class TxCurrCohortQueries {
	...
	public CohortDefinition getTxCurrCompositionCohort(String cohortName,
			boolean currentSpec) {

		final int abandonmentDays = currentSpec
				? CURRENT_SPEC_ABANDONMENT_DAYS
				: OLD_SPEC_ABANDONMENT_DAYS;
		CompositionCohortDefinition txCurrComposition = new CompositionCohortDefinition();
		txCurrComposition.setName(cohortName);

		txCurrComposition.addParameter(new Parameter("onOrBefore",
				"onOrBefore", Date.class));
		txCurrComposition.addParameter(new Parameter("location", "location",
				Location.class));
		txCurrComposition.addParameter(new Parameter("effectiveDate",
				"effectiveDate", Date.class));
		txCurrComposition.addParameter(new Parameter("locations", "location",
				Location.class));

		CohortDefinition inARTProgramAtEndDate = genericCohortQueries
				.createInProgram("InARTProgram", hivMetadata.getARTProgram());
		txCurrComposition.getSearches().put(
				"1",
				EptsReportUtils.map(inARTProgramAtEndDate,
		...
		txCurrComposition
				...
				.put("3",
						EptsReportUtils
								.map(getPatientWithHistoricalDrugStartDateObsBeforeOrOnEndDate(),
										"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition
				.getSearches()
				.put("4",
						EptsReportUtils
								.map(getPatientWithFirstDrugPickupEncounterBeforeOrOnEndDate(),
										"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put(
				"5",
				EptsReportUtils.map(
		...
		txCurrComposition
				...
				.put("8",
						EptsReportUtils
								.map(getPatientsReportedAsAbandonmentButStillInPeriod(),
										String.format(
												"onOrBefore=${onOrBefore},location=${location},abandonmentDays=%s",
												abandonmentDays)));
		txCurrComposition.getSearches().put(
				"11",
				EptsReportUtils.map(getPatientsWithNextPickupDate(),
						"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put(
				"12",
				EptsReportUtils.map(getPatientsWithNextConsultationDate(),
						"onOrBefore=${onOrBefore},location=${location}"));

		txCurrComposition.addSearch("baseCohort", EptsReportUtils.map(
				genericCohorts.getBaseCohort(),
	...
	/**
	 * @return Cohort of patients with first drug pickup (encounter type
	 *         18=S.TARV: FARMACIA) before or on end date
	 */
	...
	private CohortDefinition getPatientWithFirstDrugPickupEncounterBeforeOrOnEndDate() {
		...
		patientWithFirstDrugPickupEncounter
				.setName("patientWithFirstDrugPickupEncounter");
		...
		patientWithFirstDrugPickupEncounter.setQuery(String
				.format(query, hivMetadata.getARVPharmaciaEncounterType()
						.getEncounterTypeId()));
		patientWithFirstDrugPickupEncounter.addParameter(new Parameter(
	...
	/**
	 * @return Cohort of patients registered as START DRUGS (answer to question
	 *         1255 = ARV PLAN is 1256 = START DRUGS) in the first drug pickup
	 *         (encounter type 18=S.TARV: FARMACIA) or follow up consultation
	 *         for adults and children (encounter types 6=S.TARV: ADULTO
	 *         SEGUIMENTO and 9=S.TARV: PEDIATRIA SEGUIMENTO) before or on end
	 *         date
	 */
	...
	private CohortDefinition getPatientWithSTARTDRUGSObsBeforeOrOnEndDate() {
		...
		patientWithSTARTDRUGSObs.setQuery(String.format(query, hivMetadata
				...
				hivMetadata.getAdultoSeguimentoEncounterType()
						...
						.getEncounterTypeId(), hivMetadata.getARVPlanConcept()
						.getConceptId(), hivMetadata.getstartDrugsConcept()
						.getConceptId(), hivMetadata
						.getTransferFromOtherFacilityConcept().getConceptId()));
		patientWithSTARTDRUGSObs.addParameter(new Parameter("onOrBefore",
				"onOrBefore", Date.class));
		patientWithSTARTDRUGSObs.addParameter(new Parameter("location",
				"location", Location.class));
		return patientWithSTARTDRUGSObs;
	}

	/**
	 * @return Cohort of patients with START DATE (Concept 1190=HISTORICAL DRUG
	 *         START DATE) filled in drug pickup (encounter type 18=S.TARV:
	 *         FARMACIA) or follow up consultation for adults and children
	 *         (encounter types 6=S.TARV: ADULTO SEGUIMENTO and 9=S.TARV:
	 *         PEDIATRIA SEGUIMENTO) where START DATE is before or equal end
	 *         date
	 */
	...
	private CohortDefinition getPatientWithHistoricalDrugStartDateObsBeforeOrOnEndDate() {
		...
		patientWithHistoricalDrugStartDateObs
				.setName("patientWithHistoricalDrugStartDateObs");
		String query = "SELECT p.patient_id FROM patient p INNER JOIN encounter e ON p.patient_id=e.patient_id "
		...
		patientWithHistoricalDrugStartDateObs.setQuery(String
				.format(query, hivMetadata.getARVPharmaciaEncounterType()
						.getEncounterTypeId(), hivMetadata
		...
		patientWithHistoricalDrugStartDateObs.addParameter(new Parameter(
				"onOrBefore", "onOrBefore", Date.class));
		patientWithHistoricalDrugStartDateObs.addParameter(new Parameter(
				"location", "location", Location.class));
	 ...
	 *         Includes: dead, transferred to, stopped and abandoned (patient
	 *         state 10, 7, 8 or 9)
	 */
	...
	private CohortDefinition getPatientsWhoLeftARTProgramBeforeOrOnEndDate() {
		...
		leftARTProgramBeforeOrOnEndDate
				.setName("leftARTProgramBeforeOrOnEndDate");
		...
		String abandonStates = StringUtils.join(Arrays.asList(hivMetadata
				...
				.getProgramWorkflowStateId(), hivMetadata
		...
		leftARTProgramBeforeOrOnEndDate.setQuery(String.format(
				leftARTProgramQueryString, hivMetadata.getARTProgram()
						.getProgramId(), abandonStates));

		leftARTProgramBeforeOrOnEndDate.addParameter(new Parameter(
				"onOrBefore", "onOrBefore", Date.class));
		leftARTProgramBeforeOrOnEndDate.addParameter(new Parameter("location",
				"location", Location.class));
		return leftARTProgramBeforeOrOnEndDate;
	}

	/**
	 * @return Cohort of patients that from the date scheduled for next drug
	 *         pickup (concept 5096=RETURN VISIT DATE FOR ARV DRUG) until end
	 *         date have completed 28 days and have not returned
	 */
	...
	private CohortDefinition getPatientsThatMissedNexPickup() {
		...
		definition.setQuery(String.format(query, hivMetadata
				.getARVPharmaciaEncounterType().getEncounterTypeId(),
				hivMetadata.getReturnVisitDateForArvDrugConcept()
						.getConceptId()));
		definition.addParameter(new Parameter("onOrBefore", "onOrBefore",
				Date.class));
		definition.addParameter(new Parameter("location", "location",
				Location.class));
		definition.addParameter(new Parameter("abandonmentDays",
				"abandonmentDays", Integer.class));
		return definition;
	}

	/**
	 * @return Cohort of patients that from the date scheduled for next follow
	 *         up consultation (concept 1410=RETURN VISIT DATE) until the end
	 *         date have not completed 28 days
	 */
	...
	private CohortDefinition getPatientsThatMissNextConsultation() {
		...
		definition.setQuery(String.format(query, hivMetadata
				...
				hivMetadata.getARVPediatriaSeguimentoEncounterType()
						.getEncounterTypeId(), hivMetadata
						.getReturnVisitDateConcept().getConceptId()));
		definition.addParameter(new Parameter("onOrBefore", "onOrBefore",
				Date.class));
		definition.addParameter(new Parameter("location", "location",
				Location.class));
	 ...
	 *         workflow state is 9=ABANDONED) but from the date scheduled for
	 *         next drug pick up (concept 5096=RETURN VISIT DATE FOR ARV DRUG)
	 *         until the end date have not completed 28 days
	 */
	...
	private CohortDefinition getPatientsReportedAsAbandonmentButStillInPeriod() {
		...
		definition.setQuery(String.format(query, hivMetadata.getARTProgram()
				.getProgramId(), hivMetadata.getAbandonedWorkflowState()
				.getProgramWorkflowStateId(), hivMetadata
				.getARVPharmaciaEncounterType().getEncounterTypeId(),
				hivMetadata.getReturnVisitDateForArvDrugConcept()
						.getConceptId()));
	...
	private CohortDefinition getPatientsWithNextPickupDate() {
		...
		String encounterTypes = StringUtils.join(Arrays.asList(hivMetadata
				.getARVPharmaciaEncounterType().getEncounterTypeId()), ',');
		definition.setQuery(String.format(HAS_NEXT_APPOINTMENT_QUERY,
				hivMetadata.getReturnVisitDateForArvDrugConcept()
						.getConceptId(), encounterTypes));
		definition.addParameter(new Parameter("onOrBefore", "onOrBefore",
				Date.class));
		definition.addParameter(new Parameter("location", "location",
				Location.class));
	...
	/**
	 * Patients that have next consultation date set on their most recent
	 * encounter
	 * 
	 * @return
	 */
