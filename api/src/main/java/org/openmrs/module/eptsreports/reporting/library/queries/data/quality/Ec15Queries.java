  ...
  public static String getEc15CombinedQuery() {
    String query =
        "SELECT  "
            ...
            + "IF(pe.birthdate<date(recepcao.dataFichaRecepcao),'', DATE_FORMAT(recepcao.dataFichaRecepcao, '%d-%m-%Y %H:%i:%s')) AS data_encounter_recepcao, "
            + "IF(pe.birthdate<date(recepcao.date_created),'',DATE_FORMAT(recepcao.date_created, '%d-%m-%Y %H:%i:%s')) AS date_created, "
