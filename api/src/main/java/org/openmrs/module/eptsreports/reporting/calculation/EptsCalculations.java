...
/**
 * Utility class of common base calculations TODO: refactor needs to be merged
 * with EptsCalculationUtils
 */
public class EptsCalculations {
	 ...
	 */
	public static CalculationResultMap alive(Collection<Integer> cohort,
			PatientCalculationContext context) {
		VitalStatusDataDefinition def = new VitalStatusDataDefinition("alive");
		CalculationResultMap vitals = EptsCalculationUtils
				.evaluateWithReporting(def, cohort, null, null, context);
			...
			if (vitals.get(ptId) != null) {
				...
				alive = !vs.getDead()
						|| OpenmrsUtil.compareWithNullAsEarliest(
								vs.getDeathDate(), context.getNow()) > 0;
			}
	...
	/**
	 * Evaluates all obs of a given type of each patient TODO: refactor this to
	 * filter on patient id while fetching obs
	 * 
	 * @param concept
	 *            the obs' concept
	 * @param cohort
	 ...
	 */
	public static CalculationResultMap allObs(Concept concept,
		...
		return EptsCalculationUtils.ensureEmptyListResults(EptsCalculationUtils
				.evaluateWithReporting(def, cohort, null, null, context),
				cohort);
	}
	 ...
	 * @return the obss in a calculation result map
	 */
	public static CalculationResultMap firstObs(Concept concept,
			Collection<Integer> cohort, PatientCalculationContext context) {
		ObsForPersonDataDefinition def = new ObsForPersonDataDefinition(
				"first obs", TimeQualifier.FIRST, concept, context.getNow(),
				null);
		return EptsCalculationUtils.evaluateWithReporting(def, cohort, null,
				null, context);
	}
	 ...
	 * @return
	 */
	public static CalculationResultMap getObs(Concept concept,
			Collection<Integer> cohort, List<Location> locationList,
			List<Concept> valueCodedList, TimeQualifier timeQualifier,
			Date startDate, PatientCalculationContext context) {
		ObsForPersonDataDefinition def = new ObsForPersonDataDefinition();
	 ...
	 * @return the obss in a calculation result map
	 */
	public static CalculationResultMap lastObs(Concept concept,
			Collection<Integer> cohort, PatientCalculationContext context) {
		ObsForPersonDataDefinition def = new ObsForPersonDataDefinition(
	 ...
	 */
	public static CalculationResultMap firstProgramEnrollment(Program program,
			Collection<Integer> cohort, PatientCalculationContext context) {
		...
		CalculationResultMap results = EptsCalculationUtils
				.evaluateWithReporting(def, cohort,
	...
	public static CalculationResultMap firstEncounter(
			EncounterType encounterType, Collection<Integer> cohort,
	...
	public static CalculationResultMap firstObs(Concept question,
			Concept answer, Location location, boolean sortByDatetime,
			Collection<Integer> cohort, PatientCalculationContext context) {
		JembiObsDefinition definition = new JembiObsDefinition(
				"JembiObsDefinition");
		definition.setQuestion(question);
		definition.setAnswer(answer);
		definition.setLocation(location);
		definition.setSortByDatetime(sortByDatetime);
		return EptsCalculationUtils.evaluateWithReporting(definition, cohort,
				null, null, context);
	}

	public static CalculationResultMap firstPatientProgram(Program program,
			Location location, Collection<Integer> cohort,
			PatientCalculationContext context) {
