...
public class PoorAdherenceInLastXClinicalCalculation extends AbstractPatientCalculation {
    ...
    public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues, PatientCalculationContext context) {
        ...
        CalculationResultMap allEncountersMap = ePTSCalculationService.allEncounters(Arrays.asList(adultFollowup, childFollowup), cohort, location, context );

        for(Integer pId : cohort){
            boolean hasNoOption = false;
            ListResult patientEncounterList = (ListResult) allEncountersMap.get(pId);
            List<Encounter> encounterList = EptsCalculationUtils.extractResultValues(patientEncounterList);
            ...
            for (Encounter encounter : requiredThreeList){
                ...
                for( Obs obs : requiredObs){
                    if(obs.getConcept().equals(hivMetadata.getAdherence()) && obs.getValueCoded().equals(hivMetadata.getNoConcept()) ){
