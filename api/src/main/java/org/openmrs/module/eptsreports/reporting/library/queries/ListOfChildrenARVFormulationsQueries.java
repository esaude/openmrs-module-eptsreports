  ...
  class QUERY {
    public static final String findChildrenARVFormulations =
        "select distinct coorte12meses_final.patient_id as patient_id,  "
            ...
            + "drug.encounter_id, drug.D4,drug.D3,drug.D2,drug.D1,obs_proximo.obs_datetime proxima_levantamento from ( "
            ...
            + "group by p.patient_id  "
            + ") max_filaFinal  "
            + "inner join obs o on o.person_id=max_filaFinal.patient_id and o.concept_id=1088 and o.obs_datetime=max_filaFinal.data_fila "
            + "inner join obs obs_proximo on obs_proximo.person_id=max_filaFinal.patient_id and obs_proximo.concept_id=5096 and obs_proximo.obs_datetime=max_filaFinal.data_fila "
            + "left join  ( "
            + "select final.D1,final.D2, final.D3,final.D4,final.person_id,final.encounter_id from ( "
            + "select  "
            + "s.person_id, "
            + "rownum, "
            + "s.encounter_id, "
            + "max(case rownum when  1 then s.name end ) as D1, "
            + "max(case rownum when  2 then s.name end ) as D2, "
            + "max(case rownum when  3 then s.name end ) as D3, "
            + "max(case rownum when  4 then s.name end ) as D4 from ( "
            + "select  "
            + "final.person_id, "
            + "final.value_drug,  "
            + "name, "
            + "final.encounter_id, "
            + "@row:=if(@prev=1, @row,0) + 1 as rownum, "
            + "@prev:=1  "
            + "from ( "
            + "select final_formulacao4.obs_id,final_formulacao4.value_drug, final_formulacao4.name, final_formulacao4.person_id, final_formulacao4.encounter_id  from ( "
            + "select formulacao.obs_id,formulacao.value_drug, drug1.name,formulacao.person_id,formulacao.encounter_id from  ( "
            + "select max(grupo.obs_id) obs_id, formulacao.value_drug,formulacao.person_id,e.encounter_id from  encounter e "
            + "join obs grupo on grupo.encounter_id=e.encounter_id "
            + "join obs formulacao on formulacao.encounter_id=e.encounter_id "
            + "where formulacao.concept_id = 165256 and  grupo.concept_id =165252 and formulacao.obs_group_id = grupo.obs_id "
            + "and formulacao.voided=0 and grupo.voided=0 and e.voided=0 and e.encounter_datetime <=:endDate and e.location_id=:location "
            + "group by formulacao.person_id, formulacao.value_drug   "
            + ") formulacao "
            + "inner join drug drug1 on formulacao.value_drug=drug1.drug_id "
            + ")final_formulacao4 "
            + ") final, (SELECT @row:=0, @prev:=null) r "
            + "order by obs_id, name "
            + ") s "
            + ")final "
            + "group by person_id "
            + ") drug on drug.person_id=max_filaFinal.patient_id   "
            + "group by max_filaFinal.patient_id "
            + ") max_filaF on coorte12meses_final.patient_id=max_filaF.patient_id  "
            + "left join ( "
            + "select max_consulta.patient_id, max_consulta.data_seguimento data_consulta, obsPeso.value_numeric peso, "
            + "case obsAbordagem.value_coded "
            ...
            + "group by p.patient_id  "
            + ") max_consulta  "
            + "left join obs obsPeso on obsPeso.person_id=max_consulta.patient_id and obsPeso.concept_id=5089 and obsPeso.obs_datetime=max_consulta.data_seguimento "
            + "left join obs obsAbordagem on obsAbordagem.person_id=max_consulta.patient_id and obsAbordagem.concept_id=23725 and obsAbordagem.obs_datetime=max_consulta.data_seguimento "
            + "left join  obs obsDispensa  on obsDispensa.person_id=max_consulta.patient_id and obsDispensa.concept_id=23739 and  obsDispensa.obs_datetime=max_consulta.data_seguimento "
