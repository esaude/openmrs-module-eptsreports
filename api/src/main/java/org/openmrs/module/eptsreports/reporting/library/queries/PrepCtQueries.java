  ...
  class QUERY {
    ...
    public static final String findClientsWhoContinuePrep =
        "select patient_id from ( "
            ...
            + "and e.encounter_datetime >=:startDate and e.encounter_datetime <=:endDate and e.location_id=221 group by p.patient_id "
            + ") continuePrep "
            + "inner join person pe on pe.person_id=continuePrep.patient_id "
            + "    				where "
            + "    			((pe.birthdate is not null and timestampdiff(year,pe.birthdate,:startDate ) >= 15) or pe.birthdate is  null); ";

    public static final String findClientsPregnancy =
        "select patient_id from ( "
            + "select patient_id, max(data_gravida) pregnant_date from ( "
            ...
            + "group by patient_id "
            + ") pregnant group by patient_id "
            + ")final ";

    public static final String findClientsBreastfeeding =
        "select patient_id from ( "
            + "select patient_id, max(data_lactante) data_lactante from ( "
