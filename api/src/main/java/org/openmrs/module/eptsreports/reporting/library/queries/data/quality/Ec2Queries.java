...
public class Ec2Queries {
  ...
  public static String getEc2CombinedQuery(
    ...
    String query =
        "SELECT patient_id, NID, Name, birthdate, estimated_dob, Sex, first_entry_date, last_updated, criteria, encounter_date, date_enrolled, state FROM("
            + " SELECT brest_final.patient_id, pi.identifier AS NID, CONCAT(pn.given_name, ' ', pn.family_name ) AS Name, DATE_FORMAT(pe.birthdate, '%d-%m-%Y') AS birthdate, IF(pe.birthdate_estimated = 1, 'Yes','No') AS estimated_dob, pe.gender AS Sex, DATE_FORMAT(pe.date_created, '%d-%m-%Y %H:%i:%s') AS first_entry_date, DATE_FORMAT(pe.date_changed, '%d-%m-%Y %H:%i:%s') AS last_updated ,criteria, DATE_FORMAT(encounter_date, '%d-%m-%Y') AS encounter_date FROM( "
            + "SELECT brest.patient_id AS patient_id, brest.criteria AS criteria, MAX(brest.encounter_date) AS encounter_date FROM ("
            + "SELECT p.patient_id, 'BC1' AS criteria, e.encounter_datetime AS encounter_date "
            + " FROM patient p "
            ...
            + " AND pe.gender ='M'"
            + " UNION "
            + " SELECT  p.patient_id, 'BC2' AS criteria, e.encounter_datetime AS encounter_date "
            + " FROM patient p "
            + " INNER JOIN person pe ON p.patient_id=pe.person_id "
            + " INNER JOIN encounter e ON p.patient_id=e.patient_id "
            + " INNER JOIN obs o ON e.encounter_id=o.encounter_id "
            ...
            + " AND pe.gender ='M'"
            + " UNION "
            + " SELECT p.patient_id, 'BC3' AS criteria, e.encounter_datetime AS encounter_date "
            + " FROM patient p "
            + " INNER JOIN person pe ON p.patient_id=pe.person_id "
            + " INNER JOIN encounter e ON p.patient_id=e.patient_id "
            + " INNER JOIN obs o ON e.encounter_id=o.encounter_id "
            ...
            + " AND pe.gender ='M'"
            + " UNION "
            + " SELECT pg.patient_id, 'BC4' AS criteria, pg.date_enrolled AS encounter_date "
            + " FROM patient p "
