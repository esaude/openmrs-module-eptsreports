  ...
  class QUERY {
    ...
    public static final String findpatientwithScreeningTypeVisitAsPostTreatmentFollowUp =
        "SELECT firstCCU.patient_id FROM  ( "
            ...
            + ")firstCCU "
            + "INNER JOIN  ( "
            + "SELECT ccu.patient_id,ccu.dataccu as final_last_ccu FROM ( "
            + "SELECT p.patient_id,max(e.encounter_datetime) dataccu FROM patient p  "
            + "INNER JOIN encounter e on p.patient_id=e.patient_id "
            + "INNER JOIN obs o on o.encounter_id=e.encounter_id  "
            + "WHERE e.voided=0 and o.voided=0 and p.voided=0 AND e.encounter_type=28 "
            + "and o.concept_id=2094 and o.value_coded=703  and e.location_id=:location  "
            + "GROUP BY p.patient_id "
            + "UNION  "
            + "SELECT p.patient_id,max(e.encounter_datetime) dataccu FROM patient p "
            + "INNER JOIN encounter e on p.patient_id=e.patient_id  "
            ...
            + "GROUP BY p.patient_id  "
            + ")ccu "
            + ")lastCCUF on firstCCU.patient_id=lastCCUF.patient_id "
            + "WHERE lastCCUF.final_last_ccu <=firstCCU.dataccu "
