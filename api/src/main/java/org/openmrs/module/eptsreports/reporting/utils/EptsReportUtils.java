...
public class EptsReportUtils {
	...
	public static void purgeReportDefinition(ReportManager reportManager) {
		ReportDefinition findDefinition = findReportDefinition(reportManager
				.getUuid());
		ReportDefinitionService reportService = (ReportDefinitionService) Context
		...
		if (findDefinition != null) {
			...
			String gpName = "reporting.reportManager."
					+ reportManager.getUuid() + ".version";
			GlobalProperty gp = Context.getAdministrationService()
					.getGlobalPropertyObject(gpName);
			if (gp != null) {
	 ...
	 *             a RuntimeException if the Report Definition can't be found
	...
	public static <T extends Parameterizable> Mapped<T> map(T parameterizable,
		...
		return new Mapped<T>(parameterizable,
