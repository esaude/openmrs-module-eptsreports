...
public class EptsCalculationUtils {
	...
	/**
	 * Ensures all patients exist in a result map. If map is missing entries for
	 * any of patientIds, they are added with an empty list result
	 * 
	 * @param results
	 *            the calculation result map
	 * @param cohort
	 *            the patient ids
	 */
	 ...
	 *            the parameters for the reporting context
	 * @param calculation
	...
	public static CalculationResultMap evaluateWithReporting(
			DataDefinition dataDefinition, Collection<Integer> cohort,
			Map<String, Object> parameterValues,
			PatientCalculation calculation,
			PatientCalculationContext calculationContext) {
		try {
			EvaluationContext reportingContext = ensureReportingContext(
					calculationContext, cohort, parameterValues);
			Map<Integer, Object> data;

			if (dataDefinition instanceof PersonDataDefinition) {
				EvaluatedPersonData result = Context.getService(
						PersonDataService.class)
						.evaluate((PersonDataDefinition) dataDefinition,
								reportingContext);
				data = result.getData();
			} else if (dataDefinition instanceof PatientDataDefinition) {
				EvaluatedPatientData result = Context.getService(
						PatientDataService.class).evaluate(
						(PatientDataDefinition) dataDefinition,
						reportingContext);
				data = result.getData();
			} else {
				throw new RuntimeException("Unknown DataDefinition type: "
			...
			for (Integer ptId : cohort) {
				...
				ret.put(ptId,
						toCalculationResult(reportingResult, calculation,
								calculationContext));
			}
	...
	/**
	 * Returns the reporting {@link EvaluationContext} stored in
	 * calculationContext, creating and storing a new one if necessary. (Note:
	 * for now we never store this, and always return a new one)
	 * 
	 * @param calculationContext
	 *            the calculation context
	 * @param cohort
	...
	/**
	 * Convenience method to wrap a plain object in the appropriate calculation
	 * result subclass
	 * 
	 * @param obj
	 *            the plain object
	 ...
	 */
	protected static CalculationResult toCalculationResult(Object obj,
			PatientCalculation calculation,
			PatientCalculationContext calculationContext) {
		if (obj == null) {
			return null;
			...
			for (Object item : (Collection) obj) {
				ret.add(toCalculationResult(item, calculation,
						calculationContext));
			}
			return ret;
		} else if (obj instanceof Boolean) {
			return new BooleanResult((Boolean) obj, calculation,
					calculationContext);
		} else {
	 ...
	 */
	public static <T> T resultForPatient(CalculationResultMap results,
			Integer patientId) {
	...
	/**
	 * Convenience method to fetch a patient result as an obs TODO: refactor
	 * this since it is providing any extra functionality to resultForPatient
	 * 
	 * @param results
	 *            the calculation result map
	 * @param patientId
	 ...
	 */
	public static Obs obsResultForPatient(CalculationResultMap results,
			Integer patientId) {
	 ...
	 * 
	 * @param results
	 *            calculation result map
	 * @param requiredResult
	 *            the required result value
	...
	public static Set<Integer> patientsThatPass(CalculationResultMap results,
		...
		for (Map.Entry<Integer, CalculationResult> e : results.entrySet()) {
			...
			if ((requiredResult == null && ResultUtil.isTrue(result))
					|| (result != null && result.getValue().equals(
							requiredResult))) {
				ret.add(e.getKey());
			}
		}
	...
	/**
	 * Extracts patients from calculation result map with non-false/empty
	 * results
	 * 
	 * @param results
	 *            calculation result map
	...
	/**
	 * Extracts actual values from a list result. Always returns a list even if
	 * result is null.
	 * 
	 * @param result
	 *            the list result
	...
	public static <T> List<T> extractResultValues(ListResult result) {
		...
		if (result != null) {
			for (SimpleResult resultItem : (List<SimpleResult>) result
					.getValue()) {
