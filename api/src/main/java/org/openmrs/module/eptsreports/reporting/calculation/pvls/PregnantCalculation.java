...
public class PregnantCalculation extends AbstractPatientCalculation {
	...
	public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues,
		...
		CalculationResultMap pregnantMap = EptsCalculations.getObs(pregnant, femaleCohort, Arrays.asList(location),
		    Arrays.asList(yes), TimeQualifier.ANY, null, context);
		
		CalculationResultMap markedPregnantByWeeks = EptsCalculations.getObs(pregnantBasedOnWeeks, femaleCohort,
		    Arrays.asList(location), null, TimeQualifier.ANY, null, context);
		
		CalculationResultMap markedPregnantDueDate = EptsCalculations.getObs(pregnancyDueDate, femaleCohort,
		    Arrays.asList(location), null, TimeQualifier.ANY, null, context);
		...
		CalculationResultMap lastVl = EptsCalculations.lastObs(
		    Arrays.asList(labEncounterType, adultFollowup, pediatriaFollowup), viralLoadConcept, location, oneYearBefore,
		...
		for (Integer pId : femaleCohort) {
			...
			if (onArtForMoreThan3Months.contains(pId)) {
				...
				List<Obs> pregnantByWeeksObsList = EptsCalculationUtils.extractResultValues(pregnantByWeeksResullt);
				List<Obs> pregnantDueDateObsList = EptsCalculationUtils.extractResultValues(pregnantDueDateResult);
	...
	private boolean isPregnantByWeeks(Date lastVlDate, List<Obs> pregnantByWeeksObsList) {
		...
		for (Obs obs : pregnantByWeeksObsList) {
			if (obs.getValueNumeric() != null
			        && this.isInPregnantViralLoadRange(lastVlDate, obs.getEncounter().getEncounterDatetime())) {
				return true;
			}
		}
		return false;
	...
	private boolean isPregnantInProgram(Date lastVlDate, SimpleResult pregnantInProgramResut) {
		...
		return patientState != null && patientState.getEndDate() == null
		...
		if (result != null) {
			...
			if (patientProgram != null) {
				for (PatientState patientState : patientProgram.getStates()) {
					if (patientState.getState().equals(this.hivMetadata.getPatientIsPregnantWorkflowState())) {
						return patientState;
					}
				}
			}
