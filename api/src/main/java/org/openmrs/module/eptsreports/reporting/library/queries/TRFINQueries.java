...
public interface TRFINQueries {
  ...
  public class QUERY {
    ...
    public static final String findPatientsWhoAreTransferredInWithinReportingPeriod =
        " select patient_id, max(data_estado) from( "
            ...
            + " and ps.state= 29 and ps.end_date is null and ps.start_date>=:startDate and ps.start_date<=:endDate "
            ...
            + "and obsOpenDate.value_datetime >=:startDate and obsOpenDate.value_datetime <=:endDate and e.location_id=:location group by p.patient_id) result group by patient_id ";
