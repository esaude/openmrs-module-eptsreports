...
public class Eri4MonthsQueries {
  ...
  public static final String
    ...
    String query =
        "SELECT patient_id FROM (SELECT inicio_real.patient_id,inicio_real.data_inicio,max(lev_seg.encounter_datetime) last_encounter,IF(saida.data_estado is not null and max(lev_seg.encounter_datetime)>saida.data_estado,null,saida.state_id) final_state,IF(saida.data_estado is not null and max(lev_seg.encounter_datetime)>saida.data_estado,null,saida.data_estado) final_statedate FROM( "
            ...
            + "ps.start_date between (:endDate - INTERVAL 5 MONTH  + INTERVAL 1 DAY) AND (:endDate - INTERVAL  4 MONTH) and pg.location_id=:location "
            + "UNION "
            ...
            + "WHERE p.voided=0 AND e.voided=0 AND e.encounter_type=53 AND obsData.value_datetime BETWEEN (:endDate - INTERVAL 5 MONTH  + INTERVAL 1 DAY) AND (:endDate - INTERVAL  4 MONTH) AND e.location_id=:location "
