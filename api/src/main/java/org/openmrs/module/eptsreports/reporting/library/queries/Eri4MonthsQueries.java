...
public class Eri4MonthsQueries {
  ...
  public static final String
    ...
    String query =
        "SELECT patient_id FROM ( "
            + "SELECT inicio_real.patient_id, inicio_real.data_inicio, max(lev_seg.encounter_datetime) last_encounter, "
            ...
            + "SELECT patient_id,data_inicio FROM (Select patient_id,min(data_inicio) data_inicio FROM ( "
            ...
            + "AND e.encounter_datetime<=:endDate and e.location_id=:location GROUP BY p.patient_id UNION SELECT p.patient_id,min(value_datetime) data_inicio FROM patient p "
            + "INNER JOIN encounter e on p.patient_id=e.patient_id INNER JOIN obs o on e.encounter_id=o.encounter_id WHERE p.voided=0 and e.voided=0 and o.voided=0 and e.encounter_type in (18,6,9,53) "
            + "AND o.concept_id=1190 and o.value_datetime is not null and o.value_datetime<=:endDate and e.location_id=:location group by p.patient_id UNION "
            + "SELECT pg.patient_id,min(date_enrolled) data_inicio FROM patient p INNER JOIN patient_program pg on p.patient_id=pg.patient_id WHERE pg.voided=0 and p.voided=0 and program_id=2 "
            + "AND date_enrolled<=:endDate and location_id=:location GROUP BY pg.patient_id UNION SELECT e.patient_id, MIN(e.encounter_datetime) AS data_inicio FROM patient p "
            + "INNER JOIN encounter e on p.patient_id=e.patient_id WHERE p.voided=0 and e.encounter_type=18 AND e.voided=0 and e.encounter_datetime<=:endDate and e.location_id=:location "
            + "GROUP BY p.patient_id UNION SELECT p.patient_id,min(value_datetime) data_inicio FROM patient p INNER JOIN encounter e on p.patient_id=e.patient_id INNER JOIN obs o on e.encounter_id=o.encounter_id "
            + "WHERE p.voided=0 and e.voided=0 and o.voided=0 and e.encounter_type=52 and o.concept_id=23866 and o.value_datetime is not null and o.value_datetime<=:endDate and e.location_id=:location group by p.patient_id ) inicio "
            + "group by patient_id)inicio1 WHERE data_inicio between date_add(date_add(:endDate, interval -5 month), interval 1 day) and date_add(:endDate, interval -4 month) ) inicio_real "
            + "LEFT JOIN ( select p.patient_id, encounter_datetime FROM patient p INNER JOIN encounter e on p.patient_id=e.patient_id where 	p.voided=0 and e.voided=0 and  e.encounter_type in (6,9,18) "
            + "AND e.encounter_datetime between (:endDate - INTERVAL 5 MONTH + INTERVAL 1 DAY) and :endDate and e.location_id=:location UNION SELECT p.patient_id,value_datetime as encounter_datetime FROM patient p "
            + "INNER JOIN encounter e on p.patient_id=e.patient_id inner join obs o on e.encounter_id=o.encounter_id where p.voided=0 and e.voided=0 and o.voided=0 and e.encounter_type=52 "
            + "AND o.concept_id=23866 and o.value_datetime is not null and o.value_datetime between  (:endDate - INTERVAL 5 MONTH + INTERVAL 1 DAY) and :endDate and e.location_id=:location ) lev_seg "
            ...
            + "LEFT JOIN ( SELECT patient_id,max(data_estado) data_estado,state_id FROM ( select 	pg.patient_id, max(ps.start_date) data_estado, ps.state as state_id from patient p INNER JOIN patient_program pg "
            + "ON p.patient_id=pg.patient_id inner join patient_state ps on pg.patient_program_id=ps.patient_program_id WHERE pg.voided=0 and ps.voided=0 and p.voided=0 AND pg.program_id=2 AND ps.state in (7,8,10) "
            + "AND ps.end_date is null AND ps.start_date between  (:endDate - INTERVAL 5 MONTH + INTERVAL 1 DAY) and :endDate and location_id=:location group by pg.patient_id UNION select p.patient_id, max(o.obs_datetime) data_estado, "
            + "IF(o.value_coded=1706,7,if(o.value_coded=1366,10,8)) as state_id FROM patient p INNER JOIN encounter e on p.patient_id=e.patient_id INNER JOIN obs  o on e.encounter_id=o.encounter_id WHERE e.voided=0 and o.voided=0 "
            + "AND p.voided=0 AND e.encounter_type in (53,6) and o.concept_id in (6272,6273) and o.value_coded in (1706,1366,1709) AND o.obs_datetime between  (:endDate - INTERVAL 5 MONTH + INTERVAL 1 DAY) and :endDate and e.location_id=:location "
            + "GROUP BY p.patient_id UNION select person_id as patient_id,death_date as data_estado,10 as state_id from person where dead=1 and death_date is not null and death_date between (:endDate - INTERVAL 5 MONTH + INTERVAL 1 DAY) and :endDate "
            + "UNION SELECT p.patient_id, max(obsObito.obs_datetime) data_estado,10 as state_id from patient p inner join encounter e on p.patient_id=e.patient_id inner join obs obsEncontrado on e.encounter_id=obsEncontrado.encounter_id "
            ...
            + "AND e.encounter_datetime between (:endDate - INTERVAL 5 MONTH + INTERVAL 1 DAY) and :endDate and  e.location_id=:location and obsEncontrado.concept_id in (2003, 6348) AND obsEncontrado.value_coded=1066 AND "
            + "obsObito.concept_id=2031 and obsObito.value_coded=1383 GROUP BY p.patient_id ) allSaida group by patient_id ) saida ON inicio_real.patient_id=saida.patient_id and saida.data_estado BETWEEN inicio_real.data_inicio and :endDate "
            + "WHERE inicio_real.patient_id NOT IN ( select pg.patient_id FROM patient p  INNER JOIN patient_program pg on p.patient_id=pg.patient_id inner join patient_state ps on pg.patient_program_id=ps.patient_program_id "
            + "where pg.voided=0 and ps.voided=0 and p.voided=0 and pg.program_id=2 and ps.state=29 and ps.start_date between (:endDate - INTERVAL 5 MONTH + INTERVAL 1 DAY) AND (:endDate - INTERVAL  4 MONTH) and pg.location_id=:location "
            + "UNION SELECT p.patient_id from 	patient p inner join encounter e on p.patient_id=e.patient_id inner join obs obsTrans on e.encounter_id=obsTrans.encounter_id and obsTrans.voided=0 and obsTrans.concept_id=1369 and obsTrans.value_coded=1065 "
            + "INNER JOIN obs obsTarv on e.encounter_id=obsTarv.encounter_id and obsTarv.voided=0 and obsTarv.concept_id=6300 and obsTarv.value_coded=6276 WHERE p.voided=0 and e.voided=0 and e.encounter_type=53 "
            + "AND obsTrans.obs_datetime between (:endDate - INTERVAL 5 MONTH + INTERVAL 1 DAY) AND (:endDate - INTERVAL  4 MONTH) and e.location_id=:location ) group by inicio_real.patient_id) allPatient4Month";
  ...
  public static final String findPatientsWhoAreLostFollowUp() {
    ...
    final String query =
        "SELECT patient_id FROM ( "
            ...
            + "INNER JOIN obs obsEncontrado on e.encounter_id=obsEncontrado.encounter_id INNER JOIN obs obsObito on e.encounter_id=obsObito.encounter_id WHERE e.voided=0 and obsEncontrado.voided=0 AND p.voided=0 and obsObito.voided=0 AND e.encounter_type in (21,36,37) "
            + "AND e.encounter_datetime<=:endDate and  e.location_id=:location and obsEncontrado.concept_id in (2003, 6348) and obsEncontrado.value_coded=1066 AND obsObito.concept_id=2031 and obsObito.value_coded=1383 GROUP BY p.patient_id) allSaida GROUP BY patient_id) "
