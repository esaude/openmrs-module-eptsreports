...
public class Eri4MonthsQueries {
	 ...
	 * reporting period which include All patients who have initiated the drugs (ARV PLAN = START
	 * DRUGS) at the pharmacy or clinical visits (adults and children) by end of reporting period.
	 * All patients who have historical start drugs date set in pharmacy (FILA) or in clinical forms
	 * (Ficha de Seguimento Adulto end Ficha de seguimento Crianca ) by end of reporting period. All
	 * patients enrolled in ART Program by end of reporting period and All patients who have picked
	 * up drugs (at least one pharmacy visit) by end of reporting period
	 * 
	 * @return a union of cohort
	 */
	public static String allPatientsWhoHaveEitherClinicalConsultationOrDrugsPickupBetween61And120OfEncounterDate(
	        int arvPharmaciaEncounter, int arvAdultoSeguimentoEncounter, int arvPediatriaSeguimentoEncounter,
	        int arvPlanConcept, int startDrugsConcept, int historicalDrugsConcept, int artProgram, int transferFromStates) {
		
		return "SELECT inicio_real.patient_id" + " FROM (" + " SELECT patient_id,data_inicio" + " FROM ("
	...
	public static String allPatientsWhoInitiatedTreatmentDuringReportingPeriod(int arvPharmaciaEncounter,
	        int arvAdultoSeguimentoEncounter, int arvPediatriaSeguimentoEncounter, int arvPlanConcept,
	        int startDrugsConcept, int historicalDrugsConcept, int artProgram) {
		return "SELECT inicio_real.patient_id " + "FROM " + "(SELECT patient_id,data_inicio " + "FROM "
		        + "(SELECT patient_id,MIN(data_inicio) data_inicio " + "FROM "
		        + "(SELECT p.patient_id,min(e.encounter_datetime) AS data_inicio " + "FROM patient p "
		        + "INNER JOIN encounter e ON p.patient_id=e.patient_id "
		        + "INNER JOIN obs o ON o.encounter_id=e.encounter_id "
		        + "WHERE e.voided=0 and o.voided=0 and p.voided=0 AND " + "e.encounter_type IN ("
		        + arvPharmaciaEncounter
	...
	public static String getPatientsLostToFollowUpOnDrugPickup(int arvFarmacyEncounterType,
	        int drugPickupReturnVisitDateConcept, int daysThreshold) {
		String query = "SELECT patient_id FROM "
		        ...
		        + ":startDate and :endDate " + ") final WHERE datediff(:endDate,final.value_datetime)>=%d";
		return String.format(query, arvFarmacyEncounterType, drugPickupReturnVisitDateConcept, daysThreshold);
	}
	
	public static String getPatientsLostToFollowUpOnConsultation(int adultEncounteyType, int paedEncounterType,
	        int returnVisitConcept, int daysThreshold) {
		String query = "SELECT patient_id FROM "
		        ...
		        + " AND o.location_id=:location " + "AND encounter_datetime BETWEEN :startDate and :endDate "
		        + ") final WHERE datediff(:endDate,final.value_datetime)>=%d";
		return String.format(query, adultEncounteyType, paedEncounterType, returnVisitConcept, daysThreshold);
	}
}
