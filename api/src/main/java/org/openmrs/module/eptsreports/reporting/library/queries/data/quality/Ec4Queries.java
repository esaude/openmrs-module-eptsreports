...
public class Ec4Queries {
  ...
  public static String getEc4CombinedQuery(
    ...
    String query =
        "SELECT patient_id, NID, Name, birthdate, Estimated_dob, Sex, First_entry_date, Last_updated, date_enrolled, state, state_date, death_date, MIN(encounter_date) AS encounter_date , encounter_date_created FROM("
            ...
            + " INNER JOIN "
            + " (SELECT p.patient_id AS patientId, e.encounter_datetime AS encounter_date FROM "
            + " patient p INNER JOIN encounter e ON p.patient_id=e.patient_id WHERE p.voided = 0 and e.voided=0 "
            + " AND e.encounter_type IN("
            + adultFollowUp
            + ","
            ...
            + ") GROUP BY pa.patient_id) ec4 GROUP BY ec4.patient_id";
    return query;
  }
}
