  ...
  class QUERY {
    ...
    public static final String findPatientsWhoArePregnantCAT13Part1 =
        " select patient_id from ( "
            ...
            + " and e.encounter_datetime BETWEEN :endInclusionDate AND :endRevisionDate and "
            + " obsGravida.concept_id = 1982 and obsGravida.value_coded = 1065 and pe.gender = 'F' "
            + " group by p.patient_id) as gravida ";
    ...
    public static final String findPatientsWhoAreBreastfeedingCAT13Part1 =
        " select patient_id from ( "
            ...
            + " AND e.encounter_datetime BETWEEN :endInclusionDate AND :endRevisionDate "
            + " AND obsLactante.concept_id = 6332 AND obsLactante.value_coded = 1065 AND pe.gender = 'F' "
            + " group by p.patient_id) as lactante ";
    ...
    public static final String
        findPatientsWithLastClinicalConsultationwhoAreNotInFistLineDenominatorB2ENEW =
            " select p.patient_id FROM "
                ...
                + " join "
                + " ( "
                + " select maxEnc.patient_id, segundaLinha.encounter_datetime "
                + " from "
                + " ( "
                + " Select p.patient_id, max(e.encounter_datetime) encounter_datetime from patient p "
                + " inner join encounter e on p.patient_id = e.patient_id "
                ...
                + " inner join "
                + " ( "
                + " select p.patient_id, o.obs_datetime, e.encounter_datetime "
                + " from patient p "
                + " join encounter e on p.patient_id = e.patient_id "
                + " join obs o on o.encounter_id = e.encounter_id "
                + " where e.encounter_type = 53 and e.voided = 0 and p.voided = 0 and o.voided = 0 and o.value_coded is not null "
                ...
                + " ) segundaLinha on maxEnc.patient_id = segundaLinha.patient_id "
                + " where maxEnc.encounter_datetime >= date_add(segundaLinha.obs_datetime, INTERVAL 6 Month) "
                + " ) B2NEW ON B2NEW.patient_id = e.patient_id "
                + " inner join "
                + " ( "
                + " Select p.patient_id, max(e.encounter_datetime) encounter_datetime from patient p "
                + " inner join encounter e on p.patient_id = e.patient_id "
                ...
                + " e.encounter_datetime >= :startInclusionDate and e.encounter_datetime <= :endRevisionDate  and e.location_id = :location "
                + " group by p.patient_id "
                + " )B1 ON B1.patient_id = e.patient_id "
                + " where p.voided = 0 and e.voided = 0 and o.voided = 0 and e.encounter_type = 6 and o.concept_id = 21151 and o.value_coded <> 21150 "
                + " and o.obs_datetime > B2NEW.encounter_datetime and o.obs_datetime <= B1.encounter_datetime "
                + " group by p.patient_id ";
  }
}
