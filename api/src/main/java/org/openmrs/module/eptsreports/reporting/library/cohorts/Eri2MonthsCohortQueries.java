...
public class Eri2MonthsCohortQueries {
	...
	/**
	 * Get patients who have 2 months ART retention after ART initiation A TODO: harmonise with
	 * TxNew Union query
	 * 
	...
	public CohortDefinition getAllPatientsRetainedOnArtFor2MonthsFromArtInitiation() {
		...
		cd.setQuery(Eri2MonthsQueries.allPatientsWhoInitiatedTreatmentDuringReportingPeriod(hivMetadata
	 ...
	 * @return
	 */
	public CohortDefinition getAllPatientsWhoReturnedFor2ndConsultationOR2ndDrugsPickUpWithin33Days() {
		SqlCohortDefinition cd = new SqlCohortDefinition();
		cd.setName("Patients who picked up drugs in 33 days");
		cd.addParameter(new Parameter("startDate", "Start Date", Date.class));
		cd.addParameter(new Parameter("endDate", "End Date", Date.class));
		cd.addParameter(new Parameter("location", "Location", Location.class));
		cd.setQuery(Eri2MonthsQueries.getAllPatientsWhoReturnedFor2ndConsultationOR2ndDrugsPickUpWithin33Days(hivMetadata
		        .getARVPharmaciaEncounterType().getEncounterTypeId(), hivMetadata.getAdultoSeguimentoEncounterType()
		        .getEncounterTypeId(), hivMetadata.getARVPediatriaSeguimentoEncounterType().getEncounterTypeId(),
		    hivMetadata.getARVPlanConcept().getConceptId(), hivMetadata.getstartDrugsConcept().getConceptId(), hivMetadata
		            .gethistoricalDrugStartDateConcept().getConceptId(), hivMetadata.getARTProgram().getProgramId()));
	...
	public CohortDefinition getAllPatientsWhoInitiatedArt() {
		...
		cd.addSearch("initiatedArt", EptsReportUtils.map(getAllPatientsRetainedOnArtFor2MonthsFromArtInitiation(),
	...
	public CohortDefinition getAllPatientsWhoStartedArtAndPickedDrugsOnTheirNextVisit() {
		...
		cd.addParameter(new Parameter("endDate", "End Date", Date.class));
		cd.addParameter(new Parameter("location", "Location", Location.class));
		cd.addSearch("initiatedArt", EptsReportUtils.map(getAllPatientsWhoInitiatedArt(),
		    "cohortStartDate=${startDate},cohortEndDate=${endDate},location=${location}"));
		cd.addSearch("pickedDrugs", EptsReportUtils.map(
	...
	/**
	 * Get patients who did not pick up drugs on their second visit Did NOT pick up the drugs A and
	 * B
	 * 
	 * @return CohortDefinition
	 */
	public CohortDefinition getPatientsWhoDidNotPickDrugsOnTheirSecondVisit() {
		...
		cd.addSearch("transfers", EptsReportUtils.map(genericCohortQueries.getPatientsBasedOnPatientStates(hivMetadata
	...
	public CohortDefinition getPatientsWhoPickedUpDrugsOnTheirSecondVisit() {
		...
		cd.addParameter(new Parameter("reportingEndDate", "Reporting End Date", Date.class));
		cd.addParameter(new Parameter("location", "Location", Location.class));
		cd.addSearch("pickedDrugsAndStartedART", EptsReportUtils.map(
	...
	public CohortDefinition getPatientsWhoInitiatedArtAndDead() {
		...
		cd.addParameter(new Parameter("reportingEndDate", "Reporting End Date", Date.class));
		cd.addParameter(new Parameter("location", "Location", Location.class));
		cd.addSearch("initiatedArtAndNotTransferIns", EptsReportUtils.map(getAllPatientsWhoInitiatedArt(),
		    "cohortStartDate=${cohortStartDate},cohortEndDate=${cohortEndDate},location=${location}"));
		cd.addSearch("dead", EptsReportUtils.map(genericCohortQueries.getDeceasedPatients(),
		    "startDate=${cohortStartDate},endDate=${reportingEndDate},location=${location}"));
	...
	public CohortDefinition getPatientsWhoInitiatedArtButSuspendedTreatment() {
		...
		cd.addSearch("initiatedArtAndNotTransferIns", EptsReportUtils.map(getAllPatientsWhoInitiatedArt(),
		    "cohortStartDate=${cohortStartDate},cohortEndDate=${cohortEndDate},location=${location}"));
		cd.addSearch("suspendedTreatment", EptsReportUtils.map(genericCohortQueries.getPatientsBasedOnPatientStates(
	...
	public CohortDefinition getPatientsWhoInitiatedArtButTransferredOut() {
		...
		cd.addSearch("initiatedArtAndNotTransferIns", EptsReportUtils.map(getAllPatientsWhoInitiatedArt(),
		    "cohortStartDate=${cohortStartDate},cohortEndDate=${cohortEndDate},location=${location}"));
		cd.addSearch("transferredOut", EptsReportUtils.map(genericCohortQueries.getPatientsBasedOnPatientStates(hivMetadata
		        .getARTProgram().getProgramId(), hivMetadata.getTransferredOutToAnotherHealthFacilityWorkflowState()
		        .getProgramWorkflowStateId()),
		    "startDate=${cohortStartDate},endDate=${reportingEndDate},location=${location}"));
