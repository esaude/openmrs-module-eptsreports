...
public class BreastfeedingCalculation extends AbstractPatientCalculation {
	...
	public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues,
		...
		CalculationResultMap lactatingMap = EptsCalculations.getObs(breastfeedingConcept, femaleCohort,
		    Arrays.asList(location), Arrays.asList(yes), TimeQualifier.ANY, null, context);
		CalculationResultMap criteriaHivStartMap = EptsCalculations.getObs(criteriaForHivStart, femaleCohort,
		    Arrays.asList(location), Arrays.asList(breastfeedingConcept), TimeQualifier.ANY, null, context);
		CalculationResultMap deliveryDateMap = EptsCalculations.getObs(priorDeliveryDate, femaleCohort,
		    Arrays.asList(location), null, TimeQualifier.ANY, null, context);
		CalculationResultMap markedPregnantInProgram = EptsCalculations.lastProgramEnrollment(ptv, femaleCohort, context);
		Set<Integer> onArtForMoreThan3Months = EptsCalculationUtils.patientsThatPass(calculate(Context
		        .getRegisteredComponents(OnArtForMoreThanXmonthsCalcultion.class).get(0), femaleCohort, context));
		...
		for (Integer pId : femaleCohort) {
			...
			if (onArtForMoreThan3Months.contains(pId)) {
				...
				List<Obs> criteriaHIVStartObsList = EptsCalculationUtils.extractResultValues(criteriaHivStartResult);
				...
				if (isLactating(lastVlDate, lactatingObsList) || hasHIVStartDate(lastVlDate, criteriaHIVStartObsList)
	...
	private boolean isLactating(Date lastVlDate, List<Obs> lactatingObsList) {
		...
		for (Obs lactantObs : lactatingObsList) {
			if (isIBreastFeedingnViralLoadRange(lastVlDate, lactantObs.getEncounter().getEncounterDatetime())) {
				return true;
			}
		}
		return false;
	...
	private boolean hasHIVStartDate(Date lastVlDate, List<Obs> criteriaHIVStartObsList) {
		for (Obs hivStartDateObs : criteriaHIVStartObsList) {
			if (isIBreastFeedingnViralLoadRange(lastVlDate, hivStartDateObs.getEncounter().getEncounterDatetime())) {
				return true;
			}
		}
		return false;
	...
	private boolean isBreastFeedingInProgram(Date lastVlDate, SimpleResult pregnantInProgram) {
		...
		return patientState != null && this.isIBreastFeedingnViralLoadRange(lastVlDate, patientState.getStartDate());
	}
	...
	private boolean isIBreastFeedingnViralLoadRange(Date viralLoadDate, Date breastFeedingDate) {
		...
		return breastFeedingDate.compareTo(startDate) >= 0 && breastFeedingDate.compareTo(viralLoadDate) <= 0;
	}
}
