...
public class AgeOnArtStartDateCalculation extends AbstractPatientCalculation {
	...
	public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues,
		...
		CalculationResultMap birthDates = EptsCalculationUtils.evaluateWithReporting(new BirthdateDataDefinition(), cohort,
		    null, null, context);
		...
		if (minAge != null && maxAge != null) {
			for (Integer patientId : cohort) {
				...
				if (artStartDate != null && birthDate != null && birthDate.compareTo(artStartDate) <= 0) {
					int years = Years.yearsIn(new Interval(birthDate.getTime(), artStartDate.getTime())).getYears();
					map.put(patientId, new BooleanResult(isMinAgeOk(minAge, years) && isMaxAgeOk(maxAge, years), this));
				}
			}
			return map;
		} else {
			throw new IllegalArgumentException(String.format("Parameters %s and %s must be set", MIN_AGE, MAX_AGE));
		}
	}
