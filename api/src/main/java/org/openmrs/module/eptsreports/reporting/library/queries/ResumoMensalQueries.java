...
public class ResumoMensalQueries {
	...
	/**
	 * All patients with encounter type 53, and Pre-ART Start Date that is less than
	 * startDate
	 *
	 * @return String
	 */
	public static String getAllPatientsWithPreArtStartDateLessThanReportingStartDateA1(int encouterTypeFichaResumo,
			int conceptDataInicioPreTart, int encounterTypeAdultoInicialA, int encounterTypePediatriaInicialA,
			int programServicoTarvCuidado) {
		String query = "SELECT InitArt.patient_id FROM (SELECT p.patient_id,MIN(o.value_datetime) AS initialDate FROM patient p  "
		...
		return String.format(query, encouterTypeFichaResumo, conceptDataInicioPreTart, encounterTypeAdultoInicialA,
				encounterTypePediatriaInicialA, programServicoTarvCuidado);
	}

	/**
	 * All patients with encounter type 53, and Pre-ART Start Date that falls
	 * between startDate and enddate
	 *
	 * @return String
	 */
	public static String getAllPatientsWithPreArtStartDateWithBoundariesA2(int encouterTypeFichaResumo,
			int conceptDataInicioPreTart, int encounterTypeAdultoInicialA, int encounterTypePediatriaInicialA,
			int programServicoTarvCuidado) {
		String query = "SELECT preTarvFinal.patient_id FROM ( "
	 ...
	 * @return String
	 */
	public static String getPatientsTransferredFromAnotherHealthFacilityDuringTheCurrentMonth(int preTarvProgram,
			int trasferedState, int encouterTypeFichaResumo, int typeOfPatientTransferedFromConcept,
			int typeOfPatientTransferedFromAnswer, int transferFromOtherFacility, int transferFromOtherFacilityAnswer) {

		String query = "SELECT trasferedPatients.patient_id FROM ("
				...
				+ "UNION " + "SELECT p.patient_id,o.obs_datetime AS initialDate  FROM patient p  "
				+ "INNER JOIN encounter e  ON e.patient_id=p.patient_id "
				+ "INNER JOIN obs o on o.encounter_id=e.encounter_id "
		...
		return String.format(query, preTarvProgram, trasferedState, encouterTypeFichaResumo,
				typeOfPatientTransferedFromConcept, typeOfPatientTransferedFromAnswer, transferFromOtherFacility,
				transferFromOtherFacilityAnswer);
	}

	public static String getPatientsTransferredFromAnotherHealthFacilityDuringTheCurrentStartDateEndDate(
			int preTarvProgram, int trasferedState, int encouterTypeFichaResumo, int typeOfPatientTransferedFromConcept,
			int typeOfPatientTransferedFromAnswer, int transferFromOtherFacility, int transferFromOtherFacilityAnswer) {
		...
		String query = "SELECT trasferedPatients.patient_id FROM ( "
				...
				+ "UNION " + "SELECT p.patient_id,MIN(o.obs_datetime) AS initialDate  FROM patient p  "
				+ "INNER JOIN encounter e  ON e.patient_id=p.patient_id "
				+ "INNER JOIN obs o on o.encounter_id=e.encounter_id "
				+ "INNER JOIN obs obsPretarv on e.encounter_id=obsPretarv.encounter_id "
				+ "WHERE e.voided=0 AND o.voided=0 AND e.encounter_type=%d and obsPretarv.voided=0 and obsPretarv.concept_id=%d AND obsPretarv.value_coded=%d "
	...
	public static String getPatientsTransferredFromAnotherHealthFacilityEndDate(int preTarvProgram, int trasferedState,
			int encouterTypeFichaResumo, int typeOfPatientTransferedFromConcept, int typeOfPatientTransferedFromAnswer,
			int transferFromOtherFacility, int transferFromOtherFacilityAnswer) {
	...
	public static String getPatientsTransferredFromAnotherHealthFacilityB5() {
		String query = "select transferidopara.patient_id from ( "
	...
	public static String getPatientsWhoSuspendTratmentB6() {
		...
		String query = "select suspenso1.patient_id from ( "
	...
	public static String getPatientsWhoAbandonedTratmentB7() {
		String query = "select B7.patient_id from ( "
				...
				+ "group by p.patient_id " + "union "
				+ "select maxFila.patient_id,maxFila.data_fila data_levantamento,o.value_datetime data_proximo_levantamento from ( "
				+ "select  p.patient_id,max(encounter_datetime) as data_fila from  patient p "
				+ "inner join encounter e on p.patient_id=e.patient_id "
				+ "where encounter_type=18 and e.voided=0 and encounter_datetime <=:endDate and e.location_id=:location group by p.patient_id) maxFila "
				+ "inner join obs o on o.person_id=maxFila.patient_id "
				+ "where o.obs_datetime=maxFila.data_fila and o.concept_id=5096 and o.voided=0) maxFilaRecepcaoFinal "
				+ "group by patient_id "
	...
	public static String getPatientsWhoAbandonedTratmentB7Exclusion() {
		String query = "select B7.patient_id from ( "
				...
				+ "select  p.patient_id,max(encounter_datetime) as data_fila from  patient p "
				+ "inner join encounter e on p.patient_id=e.patient_id "
				+ "where encounter_type=18 and e.voided=0 and encounter_datetime <=:startDate and e.location_id=:location group by p.patient_id) maxFila "
				+ "inner join obs o on o.person_id=maxFila.patient_id "
				+ "where o.obs_datetime=maxFila.data_fila and o.concept_id=5096 and o.voided=0) maxFilaRecepcaoFinal "
				+ "group by patient_id "
	...
	public static String getPatientsWhoDiedTratmentB8() {
		...
		String query = "select obito.patient_id from ( " + "select patient_id,max(data_obito) data_obito from ( "
	...
	public static final String findPatientsWithAProgramStateMarkedAsTransferedInInAPeriod = "SELECT pg.patient_id FROM patient p "
			+ "INNER JOIN patient_program pg ON p.patient_id=pg.patient_id "
			+ "INNER JOIN patient_state ps ON pg.patient_program_id=ps.patient_program_id "
			+ "WHERE pg.voided=0 AND ps.voided=0 AND p.voided=0 AND pg.program_id=2 "
			+ "AND ps.state=29 AND ps.start_date=pg.date_enrolled "
			+ "AND ps.start_date <=:startDate AND location_id=:location";

	public static final String findPatientsWhoWhereMarkedAsTransferedInAndOnARTOnInAPeriodOnMasterCard = "SELECT p.patient_id from patient p "
			+ "INNER JOIN encounter e ON p.patient_id=e.patient_id "
			...
			+ "WHERE p.voided=0 AND e.voided=0 AND e.encounter_type=53 AND obsTrans.obs_datetime<=:startDate AND e.location_id=:location";

	public static final String findPatientsWhoAreNewlyEnrolledOnARTB10 = "SELECT patient_id FROM "
			...
			+ "UNION " + "SELECT pg.patient_id, MIN(date_enrolled) art_start_date FROM patient p "
			+ "INNER JOIN patient_program pg ON p.patient_id=pg.patient_id "
	...
	public static String findPatientsWhoAreCurrentlyEnrolledOnArtMOHB13() {
		String query = "select patient_id from (select  inicio_fila_seg_prox.*,GREATEST(COALESCE(data_fila,data_seguimento,data_recepcao_levantou),COALESCE(data_seguimento,data_fila,data_recepcao_levantou),COALESCE(data_recepcao_levantou,data_seguimento,data_fila))  data_usar_c, "
				+ "GREATEST(COALESCE(data_proximo_lev,data_proximo_seguimento,data_recepcao_levantou30),COALESCE(data_proximo_seguimento,data_proximo_lev,data_recepcao_levantou30),COALESCE(data_recepcao_levantou30,data_proximo_seguimento,data_proximo_lev)) data_usar from (select inicio_fila_seg.*, "
				...
				+ "GROUP BY p.patient_id " + "union Select p.patient_id,min(value_datetime) data_inicio from patient p "
				+ "inner join encounter e on p.patient_id=e.patient_id "
				+ "inner join obs o on e.encounter_id=o.encounter_id "
				...
				+ "left join ( select patient_id,max(data_estado) data_estado from ( "
				...
				+ "group by patient_id) saida on inicio.patient_id=saida.patient_id " + "left join ( "
				...
				+ "left join ( " + "Select p.patient_id,max(value_datetime) data_recepcao_levantou from patient p "
				+ "inner join encounter e on p.patient_id=e.patient_id "
				+ "inner join obs o on e.encounter_id=o.encounter_id "
				+ "where p.voided=0 and e.voided=0 and o.voided=0 and e.encounter_type=52 and  o.concept_id=23866 and o.value_datetime is not null and  o.value_datetime<=:endDate and e.location_id=:location "
	...
	public static String findPatientsWhoAreCurrentlyEnrolledOnArtMOHLastMonthB12() {
		String query = "select patient_id from (select  inicio_fila_seg_prox.*,GREATEST(COALESCE(data_fila,data_seguimento,data_recepcao_levantou),COALESCE(data_seguimento,data_fila,data_recepcao_levantou),COALESCE(data_recepcao_levantou,data_seguimento,data_fila))  data_usar_c, "
				...
				+ "Select p.patient_id,max(encounter_datetime) data_fila from patient p  "
				+ "inner join encounter e on e.patient_id=p.patient_id "
				+ "where p.voided=0 and e.voided=0 and e.encounter_type=18 and e.location_id=:location and e.encounter_datetime<=:startDate "
				+ "group by p.patient_id) max_fila on inicio.patient_id=max_fila.patient_id	"
				+ "left join (Select p.patient_id,max(encounter_datetime) data_seguimento from patient p "
				+ "inner join encounter e on e.patient_id=p.patient_id "
				...
				+ "left join obs obs_seguimento on obs_seguimento.person_id=inicio_fila_seg.patient_id and obs_seguimento.voided=0 and obs_seguimento.obs_datetime=inicio_fila_seg.data_seguimento and obs_seguimento.concept_id=1410 and obs_seguimento.location_id=:location "
				+ "group by inicio_fila_seg.patient_id) inicio_fila_seg_prox "
				+ "group by patient_id) coorte12meses_final where (data_estado is null or (data_estado is not null and  data_usar_c>data_estado)) and date_add(data_usar, interval 60 day) >=:startDate ";
		return query;
	}

	public static String findPatientWhoHaveTbSymthomsC1() {

		String query = "select tbFinal.patient_id from (" + "select tb.patient_id, obstb.obs_datetime from ( "
				+ "select p.patient_id,min(e.encounter_datetime) data_tb from patient p "
				+ "inner join encounter e on p.patient_id=e.patient_id "
				+ "inner join obs o on o.encounter_id=e.encounter_id "
				+ "where e.encounter_type=6 and  e.location_id=221 and e.voided=0 and p.voided=0 and e.encounter_datetime between :startDate and :endDate "
				+ "group by p.patient_id )tb " + "inner join obs obstb on obstb.person_id=tb.patient_id "
				+ "where obstb.concept_id=23758 and obstb.obs_datetime=tb.data_tb and obstb.voided=0 )tbFinal ";

		return query;
	}

	public static String getPatientsWhoMarkedINHC2() {
		String query = "select encounter.patient_id, encounter.encounter_datetime, obs.concept_id from encounter "
				+ "inner join patient on patient.patient_id = encounter.patient_id "
				+ "left  join obs on (obs.encounter_id = encounter.encounter_id and obs.voided =0 and obs.concept_id = 6122) "
				...
				+ "group by encounter.patient_id , encounter.encounter_datetime order by encounter.encounter_datetime";
	...
	public static String getPatientsWhoMarkedTbActiveC3() {
		String query = "select encounter.patient_id, encounter.encounter_datetime, obs.concept_id from encounter "
				+ "inner join patient on patient.patient_id = encounter.patient_id "
				+ "left  join obs on (obs.encounter_id = encounter.encounter_id and obs.voided =0 and obs.concept_id = 23761) "
				+ "where encounter.encounter_type = 6 and encounter.voided = 0 and patient.voided = 0 "
				+ "and encounter.encounter_datetime >=:startDate and encounter.encounter_datetime <=:endDate"
				+ "and encounter.location_id =:location "
				+ "group by encounter.patient_id , encounter.encounter_datetime order by encounter.encounter_datetime";
