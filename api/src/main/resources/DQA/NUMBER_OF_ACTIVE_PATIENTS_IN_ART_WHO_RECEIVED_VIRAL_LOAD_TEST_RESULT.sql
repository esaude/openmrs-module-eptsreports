        select coorte12meses_final.patient_id,coorte12meses_final.data_usar, L1.encounter_datetime from (select  inicio_fila_seg_prox.*,GREATEST(COALESCE(data_fila,data_seguimento,data_recepcao_levantou),COALESCE(data_seguimento,data_fila,data_recepcao_levantou),COALESCE(data_recepcao_levantou,data_seguimento,data_fila))  data_usar_c, 
        	...
        	(
	             ...
	             AND enc.encounter_datetime BETWEEN :startDate AND :startDate + interval 1 month - interval 1 day AND ob.concept_id IN(856,1305) 
	             ...
	             BETWEEN :startDate AND :startDate + interval 1 month - interval 1 day AND ob.concept_id IN(856, 1305) AND enc.encounter_type=6 
	             ) ed 
        	...
        	(
	             ...
	             ON p.patient_id=ed.patient_id 
	             WHERE p.voided=0 AND e.voided=0 AND o.voided=0 AND e.location_id=:location 
	             AND e.encounter_datetime BETWEEN IF(MONTH(:startDate + interval 2 month) = 12  && DAY(:startDate + interval 2 month) = 21, :startDate + interval 2 month, CONCAT(YEAR(:startDate + interval 2 month)-1, '-12','-21')) AND ((:startDate + interval 1 month) - interval 1 day) 
	             AND o.concept_id IN (856,1305)
	             AND e.encounter_type=6
	             )
