...
from 																																			
	(	select p.patient_id, o.value_datetime data_inicio_inh 																				 		
		...
		where e.voided=0 and p.voided=0 and e.encounter_datetime <= :endDateand e.location_id=:location    										
		...
		union																													 					
          ...
          from patient p 																												 		
       	...
       	where p.voided=0 and e.voided=0 and o.voided=0 and e.encounter_type in (6,9)  and o.concept_id=6122  									
       		and o.value_coded=1256 and e.encounter_datetime < :endDateand e.location_id=:location 												
       	union 																																	
       	( 																																		
			...
			from 																																	
				(	select p.patient_id, e.encounter_datetime data_inicio_inh  																		
					...
					where e.voided=0 and p.voided=0 and o.voided=0 and o.concept_id=23985 and o.value_coded in (656,23982) and e.encounter_type=60  
					 	and e.encounter_datetime < :endDateand e.location_id=:location and seguimentoTPT.obs_id is null	 						
				)  																																	
			inicio 																																	
			left join  																																
				(	select p.patient_id, o.value_datetime data_inicio_inh 																			
...
union
select termino_inh.patient_id
from
	(	select d1.patient_id 
		from	(	select distinct inicio_inh.patient_id 
				from
					(	select p.patient_id, o.value_datetime data_inicio_inh 																				
						from patient p 																													
							inner join encounter e on p.patient_id=e.patient_id 																		
